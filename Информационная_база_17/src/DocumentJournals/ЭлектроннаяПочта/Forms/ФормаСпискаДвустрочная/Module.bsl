...
Процедура ЗагрузитьНастройки()
	...
	Элементы.ФормаОткрыватьПриЗапускеПрограммы.Пометка = СтрНайти(СистемаУчетаЗадачСервер.ПриЗапускеОткрыватьРаздел(), "Почта") <> 0;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "УстановитьВыделениеНаПапкуИПисьмо" И ТипЗнч(Параметр) = Тип("Структура") Тогда
		...
		Если ТекущееПисьмо <> ПоследнееПисьмо Тогда
			...
			ПапкаПисьма = ОбщегоНазначенияСУЗВызовСервера.ЗначениеРеквизитаОбъекта(ПоследнееПисьмо, "Папка");
...
Процедура ОбработкаПерехода(ОбъектПерехода, СтандартнаяОбработка)
	...
	ПапкаПисьма = ОбщегоНазначенияСУЗВызовСервера.ЗначениеРеквизитаОбъекта(Письмо, "Папка");
...
#Если Не ВебКлиент Тогда
	//Обвноление формы писем
	НастройкиАвтообновления = Новый Структура("Автообновление, ПериодАвтоОбновления",Истина,300);
	ПодключитьОбработчикОжидания("Автообновление", НастройкиАвтообновления.ПериодАвтоОбновления, Ложь);
#КонецЕсли
...
Процедура ПереместитьВКорзину(Команда)
	...
	Если НеПеренесеноПисем > 0 Тогда
		...
		Если НеПеренесеноПисем = 1 Тогда
			ТемаПисьма = ОбщегоНазначенияСУЗВызовСервера.ЗначениеРеквизитаОбъекта(НеПеренесенныеПисьма[0], "Тема");
	...
	Если ОбластьЧтенияВключена Тогда
		...
		Если ЗначениеЗаполнено(ТекущееПисьмо) Тогда
			...
			Если РезультатОбработки.ОповеститьОПрочтении Тогда
				ОбновитьДанные(РезультатОбработки.СведенияОПапках);
			КонецЕсли;
			ПредметТекущегоПисьма = РезультатОбработки.Предмет;
			
		Иначе
			
			ОбновитьОбластьПредпросмотра("<html><body></body></html>");
			ПредметТекущегоПисьма = Неопределено;
			
		КонецЕсли;
		ЗаполнитьПредметСтрокой(ПредметТекущегоПисьма, ПредметСтрокой);			
...
Функция ОбработатьАктивизациюСтроки(
	...
	Результат.Вставить("Предмет", Письмо.Предмет);
...
Процедура УстановитьПисьмамПометкуУдаленияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли НеОбработаноПисем = 1 Тогда
		...
		ТемаПисьма = ОбщегоНазначенияСУЗВызовСервера.ЗначениеРеквизитаОбъекта(НеОбработанныеПисьма[0], "Тема");
...
Процедура ОбновитьСвойстваПапок(СведенияОПапках)
	...
	Пока ОчередьПапок.Количество() > 0 Цикл
		...
		//Определение, что есть новые письма
		Если Элемент.Выделена И Элемент.ВидПапки = ПредопределенноеЗначение("Перечисление.ВидыПапокПисем.Входящие") Тогда
	...
	Если ЕстьНовыеПисьма Тогда
		ЭтаФорма.Заголовок = НСтр("ru = 'Почта !!!'");
	Иначе
		ЭтаФорма.Заголовок = НСтр("ru = 'Почта'");
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьДанныеДляСозданияПравила(Основание)
	...
	УчетнаяЗапись = ОбщегоНазначенияСУЗВызовСервера.ЗначениеРеквизитаОбъекта(Основание, "УчетнаяЗапись");
...
Функция ДобавитьВСписокСервер(Элемент, Адресат, УчетнаяЗапись)
	...
	Если ТипЗнч(Адресат) = Тип("СправочникСсылка.АдресатыПочтовыхСообщений") Тогда
		Адрес = Адресат.Адрес;
	Иначе
		Адрес = ОбщегоНазначенияСУЗВызовСервера.ЗначениеРеквизитаОбъекта(УчетнаяЗапись, "АдресЭлектроннойПочты");
	КонецЕсли;	
...
Процедура ПометитьПапкуНаУдаление(Команда)
	...
	Пометка = ОбщегоНазначенияСУЗВызовСервера.ЗначениеРеквизитаОбъекта(Ссылка, "ПометкаУдаления");
	Если Пометка Тогда 
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Снять с ""%1"" пометку на удаление?'"),
			Строка(Ссылка));
...
Процедура ОткрыватьПриЗапускеПрограммы(Команда)
	...
	ЗначениеНастройки = ?(Элементы.ФормаОткрыватьПриЗапускеПрограммы.Пометка, "Почта-", "Почта");
...
&НаКлиенте
Процедура РазвернутьПисьмо(Команда)
	
	ИмяКнопки = ?(ТекущийЭлемент = Элементы.ПредпросмотрHTMLБезВложений, 
			"ПредпросмотрHTMLБезВложенийКонтекстноеМенюРазвернутьПисьмо_РазвернутьСвернуть", 
			"ПредпросмотрHTMLКонтекстноеМенюРазвернутьПисьмо_РазвернутьСвернуть"
	);
	Элементы[ИмяКнопки].Пометка = Не Элементы[ИмяКнопки].Пометка;
	Элементы[ИмяКнопки].Заголовок = ?(Элементы[ИмяКнопки].Пометка, "Развернуть", "Свернуть");
	
	ЗадачиКлиентСервер.РазвернутьСвернуть(ЭтаФорма, Элементы.СтраницыПредпросмотр, СтрЗаменить(ИмяКнопки, "_РазвернутьСвернуть", ""));
	
	Если Не Элементы[ИмяКнопки].Пометка Тогда
		ОтображениеПисемПереключитьНастройки();	
	КонецЕсли;
КонецПроцедуры
...
Процедура ПредметСтрокойНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ПредметТекущегоПисьма) Тогда
		ПоказатьЗначение(, ПредметТекущегоПисьма);
	ИначеЕсли ЗначениеЗаполнено(ТекущееПисьмо) Тогда
		...
		ОткрытьФорму("Документ.Задачи.ФормаВыбора", ПараметрыФормы, , , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не удалось определить текущее письмо. Выбор предмета невозможен.");
	КонецЕсли;	
	
КонецПроцедуры

&НаКлиенте
Процедура ПредметСтрокойНажатиеЗавершение(ВыбранныйПредмет, ДополнительныеПараметры) Экспорт
	
	Если ЗначениеЗаполнено(ВыбранныйПредмет) Тогда
		ПредметСтрокойНажатиеЗавершениеСервер(ВыбранныйПредмет);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ПредметСтрокойНажатиеЗавершениеСервер(ВыбранныйПредмет)
	
	ПисьмоОбъект = ТекущееПисьмо.ПолучитьОбъект();
	ПисьмоОбъект.Предмет = ВыбранныйПредмет;
	ПисьмоОбъект.Записать();
	ПредметСтрокой = Строка(ПисьмоОбъект.Предмет);
