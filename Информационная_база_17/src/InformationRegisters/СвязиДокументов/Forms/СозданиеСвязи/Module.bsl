...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВестиУчетОбращенийГраждан = Ложь;//ПолучитьФункциональнуюОпцию("ВестиУчетОбращенийГраждан"); 
...
Процедура ПолучитьНедавниеДокументы(Дерево)
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаПредмета.Наименование = ОбщегоНазначенияСУЗВызовСервера.ПредметСтрокой(Выборка.Объект);
