...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Если Параметры.Свойство("Ключ") Тогда
		...
		ЗаполнитьЗначенияСвойств(ИсходныеПараметры, Параметры);
		ИсходныеПараметры = Новый ФиксированнаяСтруктура(ИсходныеПараметры);
	//КонецЕсли;
	//Заполняется существующая запись
	МенеджерЗаписи = РегистрыСведений.НапоминанияПользователя.СоздатьМенеджерЗаписи();
	ЗаполнитьЗначенияСвойств(МенеджерЗаписи, ИсходныеПараметры);
	МенеджерЗаписи.Прочитать();
	Если МенеджерЗаписи.Выбран() Тогда
		ЗначениеВРеквизитФормы(МенеджерЗаписи,"Объект");
	КонецЕсли;
