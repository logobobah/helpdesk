...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Источник") Тогда 
		...
		Если ТипЗнч(Объект.Источник) = Тип("ДокументСсылка.Задачи") Тогда
			Объект.Описание = ?(Объект.Источник.Заказчик.Пустая(), "", Объект.Источник.Заказчик.Наименование + ". ") 
				+ ?(Объект.Источник.Проект.Пустая(), "", Объект.Источник.Проект.Наименование+". ")
				+ Объект.Описание;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЗначениеЗаполнено(Объект.ИсходныйКлючЗаписи) Тогда
		Если Элементы.ИмяРеквизитаИсточника.СписокВыбора.Количество() > 0 Тогда
			...
			Объект.СпособУстановкиВремениНапоминания = ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ВУказанноеВремя");
		КонецЕсли;
		Объект.ВремяСобытия = ОбщегоНазначенияКлиент.ДатаСеанса() + 86400; //Установка на следующий день
	КонецЕсли;
