...
Функция СохранитьИзмененияЗаписейКалендарей(ОбрабатываемыеЭлементы) Экспорт
	...
	Для Каждого ОбрабатываемыйЭлемент Из ОбрабатываемыеЭлементы Цикл
		...
		Иначе
			...
			Если ОбрабатываемыйЭлемент.ЗаписьКалендаря.ТипЗаписиКалендаря =
				ПредопределенноеЗначение("Перечисление.ТипЗаписиКалендаря.ПовторяющеесяСобытие") Тогда
				ИсключенияПовторения.Добавить(ОбрабатываемыйЭлемент.ЗаписьКалендаря);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Попытка
		...
		Для Каждого КлючИЗначение Из ПодчиненныеИсточнику Цикл
			...
			Если ОбрабатываемыйЭлемент.Свойство("ПометкаУдаления") Тогда
				...
				ИсточникОбъект.ОбновитьИсточникПриУдаленииЗаписиКалендаря(КлючИЗначение.Значение);
			Иначе
		...
		Для Каждого ОбрабатываемыйЭлемент Из НезависимыеЗаписи Цикл
			...
			Если ОбрабатываемыйЭлемент.ЗаписьКалендаря.ТипЗаписиКалендаря = Перечисления.ТипЗаписиКалендаря.ПовторяющеесяСобытие Тогда
				...
				Если ОбрабатываемыйЭлемент.Свойство("ПометкаУдаления") Тогда
					...
					Продолжить;
				КонецЕсли;
				ЗаписьКалендаряОбъект = РаботаСКалендаремСервер.СоздатьИсключениеПовторяющейсяЗаписиКалендаря(ОбрабатываемыйЭлемент);
...
Функция ПолучитьЗаписьКалендаряПоИсточнику(Источник, НомерСтрокиИсточника, Календарь) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
	               ...
	               |	И ЗаписиКалендаряСотрудника.НомерСтрокиИсточника = &НомерСтрокиИсточника
	               |	И ЗаписиКалендаряСотрудника.Календарь = &Календарь";
	Запрос.УстановитьПараметр("Источник",Источник);
	Запрос.УстановитьПараметр("НомерСтрокиИсточника",НомерСтрокиИсточника);
	Запрос.УстановитьПараметр("Календарь",Календарь);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Возврат Выборка.ЗаписьКалендаря;
	КонецЕсли;
