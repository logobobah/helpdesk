...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Объект.ОбластьПрименения = Перечисления.ОбластиПримененияШаблоновТекстов.Почта Тогда 
...
#Область РаботаСРеквизитамиНазначения

&НаКлиенте
Процедура ОбластьПримененияПриИзменении(Элемент)
	Если Объект.ОбластьПрименения.Пустая() Тогда
		Объект.ОбластьПрименения = ПредопределенноеЗначение("Перечисление.ОбластиПримененияШаблоновТекстов.Почта");
	КонецЕсли;
	СформироватьСписокРеквизитов();
КонецПроцедуры

&НаКлиенте
Процедура ОбластьПримененияОчистка(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
КонецПроцедуры
...
Процедура СформироватьСписокРеквизитов()
	...
	СведенияОШаблоне = ШаблоныТекстовСлужебный.СведенияОШаблоне(Объект);
...
Процедура ЗаполнитьДеревоРеквизитов(Приемник, Источник, ЭтоОбщиеИлиПроизвольныеРеквизиты = Неопределено)
	...
	Для Каждого СтрокаДерева Из Источник.Строки Цикл
		...
		Если ЭтоОбщиеИлиПроизвольныеРеквизиты = Неопределено Тогда
			Если СтрокаДерева.Имя = ШаблоныТекстовКлиентСервер.ЗаголовокПроизвольныхПараметров()
				ИЛИ СтрокаДерева.Имя = ШаблоныТекстовСлужебный.ЗаголовокОбщиеРеквизиты() Тогда
...
Процедура ДобавитьПараметрВТекстСообщения(Команда)
	...
	Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML") Тогда
		...
		//Элементы.ТекстПисьмаHTML.ПолучитьГраницыВыделения(ЗакладкаДляВставкиНачало, ЗакладкаДляВставкиКонец);
		//ТекстПисьма.Вставить(ЗакладкаДляВставкиКонец, Текст);
		HTMLДокумент = Элементы.ТекстПисьмаHTML.Документ;
		Выделение = HTMLДокумент.getSelection().getRangeAt(0);
...
Процедура РеквизитыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЭлементДерева.ПолучитьЭлементы().Количество() = 0 Тогда
		ДобавитьПараметрВТекстСообщения(Неопределено);
	КонецЕсли;
