...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВариантИспользования = Перечисления.ВариантыИспользованияПочты.Встроенная;
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты()
	...
	Тема = НСтр("ru = 'Тестовое сообщение 1С:Система учета задач'");
	Тело = НСтр("ru = 'Это сообщение отправлено подсистемой электронной почты 1С:Система учета задач'");
...
Процедура СоздатьУчетнуюЗапись()
	...
	Если Справочники.УчетныеЗаписиЭлектроннойПочты.ЕстьУчетнаяЗаписьСТакимАдресом(АдресЭлектроннойПочты, ВариантИспользования, Неопределено) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Адрес электронной почты не уникален'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
