...
Перем ОбновитьДанныеИсполнителяВАдреснойКниге;
...
Процедура ПередЗаписью(Отказ)
	...
	ОбновитьДанныеИсполнителяВАдреснойКниге = Ложь;
	...
	Если ЭтоНовый() Тогда
		ОбновитьДанныеИсполнителяВАдреснойКниге = Истина;
	...
	Иначе
		РеквизитыГруппыПоСсылке = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка,
			"Наименование, Недействителен, ПометкаУдаления, КонтактнаяИнформация");
			
		Если РеквизитыГруппыПоСсылке.ПометкаУдаления <> ПометкаУдаления ИЛИ РеквизитыГруппыПоСсылке.Недействителен <> Недействителен Тогда
			ОбновитьДанныеИсполнителяВАдреснойКниге = Истина;
...
Процедура ПриЗаписи(Отказ)
	...
	// Обновление адресной книги
	Если ОбновитьДанныеИсполнителяВАдреснойКниге Тогда
		Если ПолучитьФункциональнуюОпцию("ИспользоватьГруппыПользователей") И Не ЭтоНовый() Тогда
			ГруппыПользователя = Пользователи.ПолучитьГруппыПользователя(Ссылка);
			Для Каждого СтрокаГрупп Из ГруппыПользователя Цикл
				Справочники.АдреснаяКнига.ОбновитьДанныеОбъекта(
					Ссылка, СтрокаГрупп, Справочники.АдреснаяКнига.Исполнители, Ссылка);
			КонецЦикла;
		Иначе
			Справочники.АдреснаяКнига.ОбновитьДанныеОбъекта(
				Ссылка, Справочники.АдреснаяКнига.Исполнители, Справочники.АдреснаяКнига.Исполнители, Ссылка);
		КонецЕсли;
	КонецЕсли;
