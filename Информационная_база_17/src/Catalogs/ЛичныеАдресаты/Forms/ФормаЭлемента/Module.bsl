...
&НаКлиенте
Процедура ПреобразоватьВЗаказчика(Команда)
	
	ЛичныйАдресат = Объект.Ссылка;
	ПараметрыФормы = Новый Структура("ЛичныйАдресат", ЛичныйАдресат);
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПреобразоватьВЗаказчикаПродолжение",
		ЭтотОбъект,
		ПараметрыФормы);

	ОткрытьФорму("Справочник.ЛичныеАдресаты.Форма.ВыборЗаказчикаКонтактноеЛицо", ПараметрыФормы,,,,,
		ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		
КонецПроцедуры

&НаКлиенте
Процедура ПреобразоватьВЗаказчикаПродолжение(КодВозврата, Параметры) Экспорт 
	
	Если ТипЗнч(КодВозврата) = Тип("СправочникСсылка.Заказчики") 
		Или ТипЗнч(КодВозврата) = Тип("СправочникСсылка.КонтактныеЛицаЗаказчиков") Тогда
		...
		Если РаботаСЛичнымиАдресатами.ЗаполнитьКонтактИЗаменитьСсылки(ЛичныйАдресат, КодВозврата, УникальныйИдентификатор, СтрокаОшибки) Тогда
			...
			ТекстОповещения = ?(ТипЗнч(КодВозврата) = Тип("СправочникСсылка.Заказчики"),
				"заказчика", "контактное лицо");
