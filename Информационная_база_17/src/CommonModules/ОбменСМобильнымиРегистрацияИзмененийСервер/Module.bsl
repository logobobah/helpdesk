...
Процедура ВыполнитьРегистрациюДанных(МобильныйКлиент, ПараметрыСинхронизации, МассивДляРегистрации) Экспорт
	...
	ТипКлючЗначение = Тип("КлючИЗначение");

	Для каждого Элемент Из МассивДляРегистрации Цикл

		Попытка

			Если ТипЗнч(Элемент) <> ТипСтрока И ТипЗнч(Элемент) <> ТипКлючЗначение Тогда
...
Процедура ОпределитьСписокДанныхРазделаЗадачиДляРегистрации(
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	Документ.Задачи КАК ЗадачиПоИсполнителю
		|ГДЕ
		|	НЕ ЗадачиПоИсполнителю.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыЗадач.Завершена), ЗНАЧЕНИЕ(Перечисление.СтатусыЗадач.Отменена))
		|	И НЕ ЗадачиПоИсполнителю.ПометкаУдаления
		|	И (ЗадачиПоИсполнителю.Исполнитель = &Пользователь
		|			ИЛИ ЗадачиПоИсполнителю.Исполнитель = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка))
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
...
Процедура ОпределитьСписокДанныхРазделаКалендарьДляРегистрации(
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	(ЗаписиКалендаряСотрудника.Начало >= &ДатаНачалаВыгрузки
		|				И ЗаписиКалендаряСотрудника.ТипЗаписиКалендаря <> ЗНАЧЕНИЕ(Перечисление.ТипЗаписиКалендаря.ПовторяющеесяСобытие)
		|			ИЛИ ЗаписиКалендаряСотрудника.ДатаОкончанияПовторения >= &ДатаНачалаВыгрузки
		|				И ЗаписиКалендаряСотрудника.ТипЗаписиКалендаря = ЗНАЧЕНИЕ(Перечисление.ТипЗаписиКалендаря.ПовторяющеесяСобытие)
...
Функция ПолучитьУзлыОбмена(ЭлементДанных) Экспорт
	...
	//ИначеЕсли ТипЭлемента = Тип("СправочникСсылка.Заказчики")
	//	Или ТипЭлемента = Тип("СправочникСсылка.КонтактныеЛицаЗаказчиков") Тогда
	...
	ИначеЕсли ТипЭлемента = Тип("СправочникСсылка.ЗаписиКалендаряСотрудника") Тогда
	...
	ИначеЕсли ТипЭлемента = Тип("ДокументСсылка.Задачи") Тогда
...
Функция ПолучитьУзлыОбменаПоЗаписиКалендаря(ЗаписьКалендаряСсылка)
	...
	ЗапросПолучениеУзлов.Текст = 
		...
		|		ПО Мобильный.Пользователь = ЗаписиКалендаряСотрудника.Календарь.ВладелецКалендаря
...
Функция ПолучитьМассивПользователейПоЗадаче(Задача)
	...
	// Автор
	МассивПользователей.Добавить(Документы.Задачи.ПолучитьАвтора(Задача));
