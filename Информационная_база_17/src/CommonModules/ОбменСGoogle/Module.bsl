...
Функция ЗагрузитьЗаголовкиСообщенийGmail(Знач УчетнаяЗапись, Знач СеансовыеДанные, Знач РежимЗагрузки, Знач Пользователь, Знач ОтборПоКонтакту = Неопределено) Экспорт
	...
	//Если АдресаУчастников.Количество() <= 1000 Тогда
	...
	//	
	//	Если РежимЗагрузки = "РежимЗагрузкиНовыеСообщения" Тогда
...
Процедура ЗаданиеЗагрузитьКонтакты() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ЗагрузкаКонтактовИзGoogle);
...
Процедура ПриРегистрацииОбработчиковВыгрузкиДанных(ТаблицаОбработчиков) Экспорт
	...
	ОбрабатываемыеОбъектыМетаданных.Добавить(Метаданные.Документы.Задачи);
	...
	Для Каждого ТекОбъектМетаданных Из ОбрабатываемыеОбъектыМетаданных Цикл
		...
		НовыйОбработчик.Версия = МодульВыгрузкаЗагрузкаДанныхСлужебныйСобытия.ВерсияОбработчиков1_0_0_1();
	КонецЦикла;
...
Функция ТелоПакетногоЗапросаИзменениеДанныхКалендаря(ВыборкаДанных, ГраницаСекций)
	...
	Пока ВыборкаДанных.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Данные.Идентификатор)
				И Данные.ТипЗаписиКалендаря <> Перечисления.ТипЗаписиКалендаря.ЭлементПовторяющегосяСобытия Тогда
		...
		Если Данные.ТипЗаписиКалендаря = Перечисления.ТипЗаписиКалендаря.ПовторяющеесяСобытие Тогда
			ДанныеДляИзменения["recurrence"] = СформироватьСтрокиПовторенийСобытия(Данные);
		КонецЕсли;
		//Экземпляр повторяющегося события
		Если Данные.ТипЗаписиКалендаря = Перечисления.ТипЗаписиКалендаря.ЭлементПовторяющегосяСобытия Тогда
			//Поиск повторяющегося события
			ОтделитьСобытиеПовторяющегосяСобытия(ДанныеДляИзменения,Данные);
		КонецЕсли;
...
Функция ОписаниеДатыСобытия(Знач ДатаВремя)
	...
	КонецЕсли;
	Результат["timeZone"] = ЧасовойПояс();
	Возврат Результат;
...
Функция ПередаваемыеПоля(Идентификатор)
	...
	Если ЗначениеЗаполнено(Идентификатор) Тогда
		Возврат "?fields=etag,start,end,recurrence,status,summary,description,attendees,recurringEventId,originalStartTime";
	КонецЕсли;
...
Функция СобытияКалендаря(ИдентификаторКалендаря, СинхроТокен, СтраничныйТокен, СеансовыеДанные)
	...
	"https://www.googleapis.com/calendar/v3/calendars/%1/events?maxResults=256&timeZone=%2%3%4%5",
...
Функция ОписаниеПоддержкиGoogle(Форма)
	...
	ОтключенныеОбластиДоступа = РегистрыСведений.СеансовыеДанныеGoogle.ОтключенныеОбластиДоступа(ПользователиКлиентСервер.АвторизованныйПользователь());
...
Процедура ОбновитьЗаписьКалендаря(КалендарьСотрудника, ЗаписьКалендаря, Пользователь)
	...
	//Обработка события отсоединенного от повторяющегося 
	Если Не ЗначениеЗаполнено(ЗаписьОбъект.Ссылка) Тогда
		ПозицияЗнака_ = СтрНайти(ЗаписьКалендаря["id"],"_");
		Если ПозицияЗнака_ > 0 И ЗаписьКалендаря["recurringEventId"] <> Неопределено Тогда
			ЗаписьОбъект = ПоискЗаписиКалендаря(КалендарьСотрудника, ЗаписьКалендаря);
		КонецЕсли;
	КонецЕсли;
	...
	ДанныеЗаполнения.Вставить("ТипЗаписиКалендаря", Перечисления.ТипЗаписиКалендаря.Событие);
	//Повторяющееся событие
	Если ЗаписьКалендаря["recurrence"] <> Неопределено Тогда
		ЗаполнитьПоляПовторения(ДанныеЗаполнения, ЗаписьКалендаря["recurrence"]);
	КонецЕсли;
	Если ЗаписьКалендаря["recurringEventId"] <> Неопределено И Не ЗаписьОбъект.ПометкаУдаления Тогда
		ДанныеЗаполнения.Вставить("ТипЗаписиКалендаря", Перечисления.ТипЗаписиКалендаря.ЭлементПовторяющегосяСобытия);
		ДанныеЗаполнения.Вставить("recurringEventId",ЗаписьКалендаря["recurringEventId"]);
		ЗаполнитьПолеДатаВремя(ДанныеЗаполнения, "originalStartTime", ЗаписьКалендаря, "originalStartTime");
	КонецЕсли;
	...
	Если ЗаписьОбъект.ТипЗаписиКалендаря = Перечисления.ТипЗаписиКалендаря.ЭлементПовторяющегосяСобытия Тогда
		ПовторяющеесяСобытие = НайтиЗаписьПовторяющегосяСобытия(ЗаписьКалендаря["recurringEventId"]);
		Если Не ПовторяющеесяСобытие.Пустая() Тогда
			ЗаписьОбъект.ДополнительныеСвойства.Вставить("ПовторяющеесяСобытие", ПовторяющеесяСобытие);
			ЗаписьОбъект.ДополнительныеСвойства.Вставить("ДатаИсключения", ДанныеЗаполнения.originalStartTime);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПривязатьИсточникПоКлючу(ЗаписьОбъект, Пользователь)
	...
	|		ПО ЗаписиКалендаряСотрудника.Источник = Задачи.Ссылка
	|ГДЕ
...
Процедура УстановитьКалендарьПоУмолчанию(КалендарьСотрудника, ДанныеЗаполнения)
	...
	Если Не ЗначениеЗаполнено(ДанныеЗаполнения.Пользователь.ОсновнойКалендарь) Тогда
		СистемаУчетаЗадачСервер.УстановитьОсновнойКалендарь(ДанныеЗаполнения.Пользователь,КалендарьСотрудника);
	КонецЕсли;
...
Функция ИдентификаторыСообщенийДлаЗагрузкиЗаголовков(Знач УчетнаяЗапись, Знач СеансовыеДанные, Знач РежимЗагрузки, Знач ОтборПоКонтакту)
	...
	Если РежимЗагрузки = "РежимЗагрузкиПредыдущиеСообщения" Тогда
...
Процедура ЗаполнитьПоляПоРазобраннымДанным(Знач ДанныеСообщения, Знач КомпонентПакетногоОтвета)
	...
	Для Каждого ТекЗаголовок Из Заголовки Цикл
		...
		Если НРег(ТекЗаголовок["name"]) = "from" Тогда
			МассивОтправителей = ОбщегоНазначенияКлиентСервер.АдресаЭлектроннойПочтыИзСтроки(ТекЗаголовок["value"]);
			Если МассивОтправителей.Количество() > 0 Тогда
				ЗаполнитьЗначенияСвойств(ДанныеСообщения.Отправитель,МассивОтправителей);
			КонецЕсли;
		КонецЕсли;
		
		Если НРег(ТекЗаголовок["name"]) = "to" Тогда
			ДанныеСообщения.Получатели = ОбщегоНазначенияКлиентСервер.АдресаЭлектроннойПочтыИзСтроки(ТекЗаголовок["value"]);
		КонецЕсли;
...
Функция УдалитьСообщения(ИдентификаторыСообщений, Знач СеансовыеДанные)
...
Функция УдалитьМетки(ИдентификаторыСообщений, Знач СеансовыеДанные, Знач ИдентификаторМетки)
...
Функция ЗаписатьСобытиеПоДаннымGMail(ДанныеСообщения, УчетнаяЗапись, СопоставленныеКонтакты, Пользователь, ВариантЗагрузки)
	...
	//!!! Из календаря Гугл должны перегружаться только изменения по существующим задачам
	//Новый события из гугл календаря перегружаться не должны.
	
	Результат = Документы.Задачи.ПустаяСсылка();
	
	ИдентификаторыСвязанныхСообщений = ИдентификаторыСвязанныхСообщений(ДанныеСообщения.ИдентификаторыОснований);
...
Функция ИдентификаторыСвязанныхСообщений(СтрокаИдентификаторов) Экспорт
	...
	Для Каждого ТекЭлемент Из СтрРазделить(СтрокаИдентификаторов, " ") Цикл
		ТекИдентификатор = СократитьУгловыеСкобки(ТекЭлемент);
...
Функция СформироватьСтрокиПовторенийСобытия(Событие)
	
	Повторения = Новый Массив;
	//Варианты ежедневно
	BYDAY="";
	BYMONTHDAY = "";
	Если Событие.ЧастотаПовторения = Перечисления.ЧастотаПовторения.Ежедневно Тогда
		FREQ = "DAILY";
	ИначеЕсли Событие.ЧастотаПовторения = Перечисления.ЧастотаПовторения.Еженедельно Тогда
		FREQ = "WEEKLY";
		ДниНеделиГугл = ДниНеделиГугл();
		Для Каждого СтрокаТЧ Из Событие.ПовторениеПоДням Цикл
			 BYDAY = BYDAY + ДниНеделиГугл.Получить(СтрокаТЧ.ДеньНедели)+",";
		КонецЦикла;
		Если Не ПустаяСтрока(BYDAY) Тогда
			BYDAY = Лев(BYDAY,СтрДлина(BYDAY)-1)+";";
		КонецЕсли;
	ИначеЕсли Событие.ЧастотаПовторения = Перечисления.ЧастотаПовторения.Ежемесячно Тогда
		FREQ = "MONTHLY";
		Если Событие.ПовторениеПоДнямМесяца > 0 Тогда
			BYMONTHDAY = Формат(Событие.ПовторениеПоДнямМесяца,"ЧГ=0")+";";
		Иначе
			ДниНеделиГугл = ДниНеделиГугл();
			Для Каждого СтрокаТЧ Из Событие.ПовторениеПоДням Цикл
				 BYDAY = BYDAY + Формат(СтрокаТЧ.НомерВхождения,"ЧГ=0") + ДниНеделиГугл.Получить(СтрокаТЧ.ДеньНедели)+",";
			КонецЦикла;
			Если Не ПустаяСтрока(BYDAY) Тогда
				BYDAY = Лев(BYDAY,СтрДлина(BYDAY)-1)+";";
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли Событие.ЧастотаПовторения = Перечисления.ЧастотаПовторения.Ежегодно Тогда
		FREQ = "YEARLY";
	Иначе
		Возврат Повторения;
	КонецЕсли;
	...
	ТекстЗавершения = "";
	Если Событие.ПравилоОкончанияПовторения = Перечисления.ПравилаОкончанияПовторения.ДоДаты Тогда
		ТекстЗавершения = "UNTIL="+Формат(Событие.ДатаОкончанияПовторения,"ДФ=yyyyMMddTHHmmssZ")+";";
	ИначеЕсли Событие.ПравилоОкончанияПовторения = Перечисления.ПравилаОкончанияПовторения.ПослеЧислаПовторов Тогда
		ТекстЗавершения = "COUNT="+Формат(Событие.КоличествоПовторов,"ЧГ=0")+";";
	КонецЕсли;
	
	СтрокаПовторения = "RRULE:FREQ="+FREQ+";"  //Частота
		+ ?(Событие.ИнтервалПовторения>0, "INTERVAL="+Формат(Событие.ИнтервалПовторения,"ЧГ=0")+";", "") //Интервал с которым повторяется запись календаря 
		+ ТекстЗавершения //До какой даты или какое количество повторов
		+ ?(ПустаяСтрока(BYDAY),"","BYDAY="+BYDAY) //Повторение по дням недели, дням недели в месяце
		+ ?(ПустаяСтрока(BYMONTHDAY),"","BYMONTHDAY="+BYMONTHDAY); //Повторение по дням месяца
	СтрокаПовторения = Лев(СтрокаПовторения,СтрДлина(СтрокаПовторения)-1);
	
	Повторения.Добавить(СтрокаПовторения);
	
	//Добавление дней исключений.
	//Если Событие.ИсключенияПовторения.Количество() > 0 Тогда
	//	Даты = "";
	//	Для Каждого СтрокаТЧ Из Событие.ИсключенияПовторения Цикл
	//		Даты = Даты + Формат(СтрокаТЧ.ДатаИсключения,"ДФ=yyyyMMdd")+",";
	//	КонецЦикла;
	//	Даты = Лев(Даты,СтрДлина(Даты)-1);
	//	Повторения.Добавить("EXDATE;VALUE=DATE:"+Даты);
	//КонецЕсли;
	
	Возврат Повторения;
КонецФункции

Функция ДниНеделиГугл()
	Соответствие = Новый Соответствие;
	Соответствие.Вставить(1,"MO");
	Соответствие.Вставить(2,"TU");
	Соответствие.Вставить(3,"WE");
	Соответствие.Вставить(4,"TH");
	Соответствие.Вставить(5,"FR");
	Соответствие.Вставить(6,"SA");
	Соответствие.Вставить(7,"SU");
	Возврат Соответствие;
КонецФункции

Процедура ЗаполнитьПоляПовторения(ДанныеЗаполнения, МассивRecurrence)
	
	Если МассивRecurrence = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаRRULE = ""; //RRULE:FREQ=WEEKLY;WKST=MO;COUNT=2;BYDAY=2SA,SU   регулярное правило для повторения события
	СтрокаRDATE = ""; //RDATE;VALUE=DATE:19970101,19970120              дополнительные даты или дату и время , когда вхождения события должны произойти
	СтрокаEXDATE = "";//EXDATE;VALUE=DATE:20150610                      даты или даты-времени , когда событие должно не произойти
	Для Каждого СтрокаМассива Из МассивRecurrence Цикл
		Если Лев(СтрокаМассива,5) = "RRULE" Тогда
			СтрокаRRULE = Сред(СтрокаМассива,7);
		ИначеЕсли Лев(СтрокаМассива,5) = "RDATE" Тогда
			СтрокаRDATE = Сред(СтрокаМассива,7);
		ИначеЕсли Лев(СтрокаМассива,6) = "EXDATE" Тогда
			СтрокаEXDATE = Сред(СтрокаМассива,8);
		КонецЕсли;
	КонецЦикла;
	
	Если Не ПустаяСтрока(СтрокаRRULE) Тогда
		ДанныеЗаполнения.Вставить("ТипЗаписиКалендаря", Перечисления.ТипЗаписиКалендаря.ПовторяющеесяСобытие);
		ДанныеЗаполнения.Вставить("ДатаНачалаПовторения", ДанныеЗаполнения.Начало);
		ДанныеЗаполнения.Вставить("ДатаОкончанияПовторения", Дата(1,1,1)); 
		ДанныеЗаполнения.Вставить("ПравилоОкончанияПовторения",Перечисления.ПравилаОкончанияПовторения.Никогда);
		ДанныеЗаполнения.Вставить("ИнтервалПовторения",1);
		
		ПовторениеПоДням = Новый ТаблицаЗначений;
		ПовторениеПоДням.Колонки.Добавить("ДеньНедели");
		ПовторениеПоДням.Колонки.Добавить("НомерВхождения");
		
		МассивКомандСтроки = СтрРазделить(СтрокаRRULE,";",Ложь);
		Для Каждого Команда Из МассивКомандСтроки Цикл
			//Частота повторения
			Если Лев(Команда,4) = "FREQ" Тогда
				ДанныеЗаполнения.Вставить("ЧастотаПовторения",ПолучитьЧастотуПовторения(Сред(Команда,6)));
			ИначеЕсли Лев(Команда,8) = "INTERVAL" Тогда
				ДанныеЗаполнения.Вставить("ИнтервалПовторения",Число(Сред(Команда,10)));
			ИначеЕсли Лев(Команда,5) = "UNTIL" Тогда
				ДатаОкончанияТекст = СтрЗаменить(СтрЗаменить(Сред(Команда,7),"T",""),"Z","");
				ДанныеЗаполнения.Вставить("ДатаОкончанияПовторения",Дата(ДатаОкончанияТекст));
				ДанныеЗаполнения.Вставить("ПравилоОкончанияПовторения",Перечисления.ПравилаОкончанияПовторения.ДоДаты);
			ИначеЕсли Лев(Команда,5) = "COUNT" Тогда
				ДанныеЗаполнения.Вставить("КоличествоПовторов",Число(Сред(Команда,7)));
				ДанныеЗаполнения.Вставить("ПравилоОкончанияПовторения",Перечисления.ПравилаОкончанияПовторения.ПослеЧислаПовторов);
			ИначеЕсли Лев(Команда,10) = "BYMONTHDAY" Тогда
				ДанныеЗаполнения.Вставить("ПовторениеПоДнямМесяца",Число(Сред(Команда,12)));
			//Данный паарметр зависи от FREQ
			ИначеЕсли Лев(Команда,5) = "BYDAY" Тогда
				МассивДней = СтрРазделить(Сред(Команда,7),",",Ложь);
				Для Каждого ДеньСтрока Из МассивДней Цикл
					ДеньНомер = РазбитьДеньНомер(ДеньСтрока);
					НоваяСтрока = ПовторениеПоДням.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока,ДеньНомер);
				КонецЦикла;
			КонецЕсли;
			
		КонецЦикла;
		
		Если ДанныеЗаполнения.ЧастотаПовторения = Перечисления.ЧастотаПовторения.Ежегодно Тогда
			ДанныеЗаполнения.Вставить("ПовторениеПоМесяцам",Месяц(ДанныеЗаполнения.ДатаНачалаПовторения));
		ИначеЕсли ДанныеЗаполнения.ЧастотаПовторения = Перечисления.ЧастотаПовторения.Ежемесячно
				И ПовторениеПоДням.Количество() = 0 Тогда
			ДанныеЗаполнения.Вставить("ПовторениеПоДнямМесяца", День(ДанныеЗаполнения.ДатаНачалаПовторения));
		КонецЕсли;
		Если ПовторениеПоДням.Количество() > 0 Тогда
			ДанныеЗаполнения.Вставить("ПовторениеПоДням",ПовторениеПоДням);
		КонецЕсли;
	КонецЕсли;
	
	Если Не ПустаяСтрока(СтрокаEXDATE) тогда
		ИсключенияПовторения = Новый ТаблицаЗначений;
		ИсключенияПовторения.Колонки.Добавить("ДатаИсключения");
		
		//EXDATE;VALUE=DATE:20150610
		ДатыСтрокой = СтрЗаменить(СтрокаEXDATE,"VALUE=DATE:","");
		МассивДней = СтрРазделить(ДатыСтрокой,",",Ложь);
		Для Каждого СтрокаМассива Из МассивДней Цикл
			НоваяСтрока = ИсключенияПовторения.Добавить();
			НоваяСтрока.ДатаИсключения = Дата(СтрокаМассива);
		КонецЦикла;
		Если ИсключенияПовторения.Количество() > 0 Тогда
			ДанныеЗаполнения.Вставить("ИсключенияПовторения",ИсключенияПовторения);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Функция ПолучитьЧастотуПовторения(ЧастотаТекст)
	Частота = Перечисления.ЧастотаПовторения.Ежегодно;
	Если ЧастотаТекст = "DAILY" Тогда
		Частота = Перечисления.ЧастотаПовторения.Ежедневно;
	ИначеЕсли ЧастотаТекст = "WEEKLY" Тогда
		Частота = Перечисления.ЧастотаПовторения.Еженедельно;
	ИначеЕсли ЧастотаТекст = "MONTHLY" Тогда
		Частота = Перечисления.ЧастотаПовторения.Ежемесячно;
	КонецЕсли;
	Возврат Частота;
КонецФункции
...
Функция ПолучитьНомерДняНедели(ДеньНедели)
	День = 1;
	Если ДеньНедели = "TU" Тогда
		День = 2;
	ИначеЕсли ДеньНедели = "WE" Тогда
		День = 3;
	ИначеЕсли ДеньНедели = "TH" Тогда
		День = 4;
	ИначеЕсли ДеньНедели = "FR" Тогда
		День = 5;
	ИначеЕсли ДеньНедели = "SA" Тогда
		День = 6;
	ИначеЕсли ДеньНедели = "SU" Тогда
		День = 7;
	КонецЕсли;
	Возврат День;
КонецФункции	

Функция РазбитьДеньНомер(ДеньСтрока)
	Результат = Новый Структура("ДеньНедели,НомерВхождения");
	
	//Поиск в строке дня недели
	ДеньНедели = Прав(ДеньСтрока,2);
	НомерВхожденияСтрока = СтрЗаменить(ДеньСтрока,ДеньНедели,"");
	Если ПустаяСтрока(НомерВхожденияСтрока) Тогда
		Результат.НомерВхождения = 0;
	Иначе
		Результат.НомерВхождения = Число(НомерВхожденияСтрока);
	КонецЕсли;
	Результат.ДеньНедели = ПолучитьНомерДняНедели(ДеньНедели);
	Возврат Результат;
КонецФункции

Процедура ОтделитьСобытиеПовторяющегосяСобытия(ДанныеДляИзменения, Событие)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЗаписиКалендаряСотрудника.Ссылка КАК ПовторяющеесяСобытие
	               |ИЗ
	               |	Справочник.ЗаписиКалендаряСотрудника КАК ЗаписиКалендаряСотрудника
	               |ГДЕ
	               |	ЗаписиКалендаряСотрудника.Идентификатор ПОДОБНО &Идентификатор";
	Запрос.УстановитьПараметр("Идентификатор",Событие.ИдентификаторПовторяющегосяСобытия);
	Выборка = Запрос.Выполнить().Выбрать();
	Если НЕ Выборка.Следующий() Тогда
		Возврат;
	КонецЕсли;

	ДанныеДляИзменения["recurringEventId"] = Событие.ИдентификаторПовторяющегосяСобытия;//Выборка.ПовторяющеесяСобытие.Идентификатор;
	ИзначальнаяДатаНачала = НачалоДня(Событие.Начало) + (Выборка.ПовторяющеесяСобытие.Начало - НачалоДня(Выборка.ПовторяющеесяСобытие.Начало));
	ДанныеДляИзменения["originalStartTime"] = ОписаниеДатыСобытия(ИзначальнаяДатаНачала);
	//ДанныеДляИзменения["id"] = Событие.Идентификатор;
	
КонецПроцедуры

Функция НайтиЗаписьПовторяющегосяСобытия(Идентификатор)
...
Функция ПоискЗаписиКалендаря(КалендарьСотрудника, ЗаписьКалендаря)	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ЗаписиКалендаряСотрудника.Ссылка КАК Ссылка
	|ИЗ
	...
	|	И ЗаписиКалендаряСотрудника.ИдентификаторПовторяющегосяСобытия = &ИдентификаторПовторяющегосяСобытия
	|	И ЗаписиКалендаряСотрудника.Идентификатор ПОДОБНО &Идентификатор + ""%""");
	Запрос.УстановитьПараметр("ИдентификаторПовторяющегосяСобытия", ЗаписьКалендаря["recurringEventId"]);
	Идентификатор = ЗаписьКалендаря["id"];
	Запрос.УстановитьПараметр("Идентификатор", Лев(Идентификатор,СтрДлина(Идентификатор)-8));
	Запрос.УстановитьПараметр("Календарь", КалендарьСотрудника);
