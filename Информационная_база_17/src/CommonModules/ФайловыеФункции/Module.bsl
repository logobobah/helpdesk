...
Процедура ДобавитьНаДиск(
	...
	Для Каждого СтрокаТома Из ТаблицаТомов Цикл
		...
		ПутьКТому = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ПутьКТому, ОбщегоНазначенияСУЗПовтИсп.ТипПлатформыСервера());
		...
		Попытка
			...
			ИмяФайлаСПутем = ФайловыеФункцииКлиентСервер.ПолучитьУникальноеИмяСПутем(ПутьКТому, ИмяФайла, ОбщегоНазначенияСУЗПовтИсп.ТипПлатформыСервера());
