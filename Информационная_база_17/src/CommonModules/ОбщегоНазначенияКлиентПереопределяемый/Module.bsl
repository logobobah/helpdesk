...
Процедура ПослеНачалаРаботыСистемы() Экспорт
	...
	Если ПараметрыРаботыКлиента.Свойство("ПриЗапускеОткрыватьРаздел") Тогда
		...
		Если СтрНайти(Нрег(ПараметрыРаботыКлиента.ПриЗапускеОткрыватьРаздел), "доска") > 0 Тогда
			ОткрытьФорму("Обработка.Доска.Форма.Форма");
		КонецЕсли; 
		Если СтрНайти(Нрег(ПараметрыРаботыКлиента.ПриЗапускеОткрыватьРаздел), "задачи") > 0 Тогда
			ОткрытьФорму("Документ.Задачи.ФормаСписка");
		КонецЕсли; 
		Если СтрНайти(Нрег(ПараметрыРаботыКлиента.ПриЗапускеОткрыватьРаздел), "календарь") > 0 Тогда
			ОткрытьФорму("Справочник.ЗаписиКалендаряСотрудника.Форма.Календарь");
		КонецЕсли; 
		Если СтрНайти(Нрег(ПараметрыРаботыКлиента.ПриЗапускеОткрыватьРаздел), "почта") > 0 Тогда
			ИмяФормы = "ФормаСписка";
			Если ВстроеннаяПочтаСервер.ПолучитьПерсональнуюНастройку("ОтображениеОбластиЧтения") = 
					ПредопределенноеЗначение("Перечисление.ВариантыОтображенияОбластиЧтения.Справа") Тогда
				ИмяФормы = "ФормаСпискаДвустрочная";
			КонецЕсли;
			ОткрытьФорму("ЖурналДокументов.ЭлектроннаяПочта.Форма."+ИмяФормы);
		КонецЕсли; 
		
		//Активизирование начальной страницы
