...
Процедура ОбработатьДанныеОбъектаDMX(ДанныеОбъекта, ПисьмаКОтправке, СвязиКУстановке, ПараметрыСинхронизации) Экспорт
	...
	ИначеЕсли СтрСравнить(ИмяТипа, "Task") = 0 Тогда
		ЗаписатьЗадачу(ДанныеОбъекта, МобильныйКлиент, ПараметрыСинхронизации);
...
Процедура ЗаписатьЗаписьРабочегоКалендаряПоВерсии206(ДанныеОбъекта, МобильныйКлиент)
	...
	Ссылка = Справочники.ЗаписиКалендаряСотрудника.ПолучитьСсылку(
		ПолучитьЗначениеРеквизита(ДанныеОбъекта, "ID", "УникальныйИдентификатор"));
	Объект = Ссылка.ПолучитьОбъект();

	Если Объект = Неопределено Тогда
		Объект = Справочники.ЗаписиКалендаряСотрудника.СоздатьЭлемент();
	...
	Объект.Начало    = ПолучитьЗначениеРеквизита(ДанныеОбъекта, "StartDate", "Дата");
	Объект.Окончание = ПолучитьЗначениеРеквизита(ДанныеОбъекта, "EndDate", "Дата", Ложь);
	...
	Если ЗначениеЗаполнено(СсылкаПредмет) И Не ЗначениеЗаполнено(Объект.Источник) Тогда
		Объект.Источник = СсылкаПредмет;
	КонецЕсли;
		...
		Если Не ТаблицаДни = Неопределено Тогда
			...
			Для Каждого СтрокаТаблица Из ТаблицаДни Цикл
				...
				НоваяСтрока.ЗаписьИсключения = 
					Справочники.ЗаписиКалендаряСотрудника.ПолучитьСсылку(
...
Процедура ЗаписатьФайлПоВерсии206(ДанныеОбъекта)
	...
	ИначеЕсли СтрСравнить(ВладелецТип, "Task") = 0 Тогда
		// получаем по id Задачу.
		ВладелецФайла = Документы.Задачи.ПолучитьСсылку(ВладелецИд);
...
Процедура ЗаписатьЗадачу(ДанныеОбъекта, МобильныйКлиент, ПараметрыСинхронизации)
	...
	Ссылка = Документы.Задачи.ПолучитьСсылку(
		ПолучитьЗначениеРеквизита(ДанныеОбъекта, "ID", "УникальныйИдентификатор"));
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Задача.УстановитьСсылкуНового(Ссылка);
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ВерсияДанных) И Не Задача.ВерсияДанных = ВерсияДанных Тогда

			ОбменСМобильнымиРегистрацияИзмененийСервер.ВыполнитьРегистрациюЭлементаДанных(Задача.Ссылка);

			ТекстПредупреждения = 
				"warning:" + 
				СтрШаблон(
					НСтр("ru = 'Задача ""%1"" не была передана, т.к. она была изменена на сервере. 
								|Задача на данном устройстве заменена версией задачи с сервера.'"),
					ПолучитьЗначениеРеквизита(ДанныеОбъекта, "PRESENTATION", "Строка", Ложь));
	...
	Задача.Наименование = ПолучитьЗначениеРеквизита(ДанныеОбъекта, "Presentation", "Строка");
	Задача.Статус = Перечисления.СтатусыЗадач[ПолучитьЗначениеРеквизита(ДанныеОбъекта, "Status", "Строка")];
	Задача.Приоритет = ПолучитьЗначениеРеквизита(ДанныеОбъекта, "Importance", "Строка", Ложь);
	Задача.Описание = ПолучитьЗначениеРеквизита(ДанныеОбъекта, "Description", "Строка", Ложь);
	Задача.РезультатВыполнения = ПолучитьЗначениеРеквизита(ДанныеОбъекта, "ExecutionResult", "Строка", Ложь);
	Задача.Срок = ПолучитьЗначениеРеквизита(ДанныеОбъекта, "DeadLine", "Дата", Ложь);
	Задача.ОценкаТрудозатрат = ПолучитьЗначениеРеквизита(ДанныеОбъекта, "Cost", "Число", Ложь);

	Если Задача.Исполнитель.Пустая() Тогда
		Задача.Исполнитель = ПользователиКлиентСервер.АвторизованныйПользователь();
	КонецЕсли;

	// заполнение получателей письма
	Задача.Работы.Очистить();

	Работы = ПолучитьЗначениеРеквизита(ДанныеОбъекта, "Works" , , Ложь);
	Если Не Работы = Неопределено Тогда

		Для Каждого СтрокаТаблица Из Работы Цикл

			ДанныеСтроки = ПолучитьДанныеСтроки(Работы, СтрокаТаблица);

			НоваяСтрока = Задача.Работы.Добавить();
			НоваяСтрока.ДатаНачала = ПолучитьЗначениеРеквизита(ДанныеСтроки, "StartDate", "Дата", Ложь);
			НоваяСтрока.ДатаОкончания = ПолучитьЗначениеРеквизита(ДанныеСтроки, "FinishDate", "Дата", Ложь);
			НоваяСтрока.Трудозатраты = ПолучитьЗначениеРеквизита(ДанныеСтроки, "LaborCosts", "Число", Ложь);
			НоваяСтрока.ВыполненныеДействия = ПолучитьЗначениеРеквизита(ДанныеСтроки, "CommentWork", "Строка", Ложь);
