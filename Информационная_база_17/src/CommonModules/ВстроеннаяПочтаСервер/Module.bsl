...
Функция ПолучитьИОтправитьПисьма() Экспорт
	...
	ПараметрыЛогирования = Новый Структура("ИдентификаторСеанса, ПорядковыйНомерСобытия",
		ИдентификаторСеанса, ПорядковыйНомерСобытия);
...
Процедура ПолучитьПодчиненныеПисьма(Письмо, МассивПисем) Экспорт
	...
	ИначеЕсли ВстроеннаяПочтаКлиентСервер.ЭтоИсходящееПисьмо(Письмо) Тогда
		...
		Для каждого Связь Из ВсеСвязиДокумента Цикл
			...
			Если (ТипСвязи = Справочники.ТипыСвязей.ПолученоОтветноеПисьмо
				Или ТипСвязи = Справочники.ТипыСвязей.ПересланоПисьмом
...
Функция ПолучитьФайлыПисьма(
	...
	Запрос = Новый Запрос(
		...
		|	Файлы.Наименование + ""."" + Файлы.Расширение КАК Представление,
		...
		|	Файлы.ДатаМодификацииУниверсальная КАК ТекущаяВерсияДатаМодификацииФайла,
...
Функция ПолучитьТекстHTMLДляФото(Шапка, УникальныйИдентификатор)
	...
	Если ЗначениеЗаполнено(Шапка.ОтправительКонтакт) Тогда
		...
		//ПриложениеЯвляетсяВебКлиентом = ОбщегоНазначенияСУЗ.ПриложениеЯвляетсяВебКлиентом();
		//
...
//
Функция СформироватьТекстИсходящегоПисьма(Основание, ТипТекста, Кодировка, ТипОтвета, ИспользованныйШаблон = Неопределено, Тема = "") Экспорт
	
	ТекстПисьма = "";
	ЭтоШаблонПисьма = Ложь;
	
	Если ДелопроизводствоКлиентСервер.ЭтоПроект(Основание) 
			ИЛИ ТипЗнч(Основание) = Тип("СправочникСсылка.Заказчики")
			ИЛИ ТипЗнч(Основание) = Тип("СправочникСсылка.ШаблоныЗадач") Тогда
		
		ТекстПисьма = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "Описание");
		ТекстПисьма = РаботаС_HTML.ПолучитьHTMLИзТекста(ТекстПисьма);
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("СправочникСсылка.ШаблоныПисем") Тогда 
		...
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основание, 
			"Тема, ТипТекста, Ссылка, ТекстШаблонаПисьма, ТекстШаблонаПисьмаHTML, Кодировка");
	...
	ИначеЕсли ДелопроизводствоКлиентСервер.ЭтоЗадачаИсполнителя(Основание) Тогда
		...
		ОписаниеЗадачи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "Описание");
		РезультатЗадачи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "РезультатВыполнения");
		ТекстПисьма = "";
		Если Не ПустаяСтрока(ОписаниеЗадачи) Тогда
			ТекстПисьма = ТекстПисьма + "Описание задания: " + Символы.ПС + Символы.ПС;
			ТекстПисьма = ТекстПисьма + ОписаниеЗадачи;
		КонецЕсли;
		Если Не ПустаяСтрока(ТекстПисьма) Тогда
			ТекстПисьма = ТекстПисьма + Символы.ПС + Символы.ПС;
		КонецЕсли;
		Если Не ПустаяСтрока(РезультатЗадачи) Тогда
			ТекстПисьма = ТекстПисьма + "Комментарии к выполнению: " + Символы.ПС + Символы.ПС;
			ТекстПисьма = ТекстПисьма + РезультатЗадачи;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("СправочникСсылка.Пользователи")
			ИЛИ ТипЗнч(Основание) = Тип("СправочникСсылка.КонтактныеЛицаЗаказчиков") Тогда 
		
		ТекстПисьма  = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "Комментарии");
		ТекстПисьма = РаботаС_HTML.ПолучитьHTMLИзТекста(ТекстПисьма);
	...
	Иначе 
		...
		Если ТипЗнч(ШаблонПисьма) = Тип("СправочникСсылка.ШаблоныТекстов")
			...
			Подпись = Справочники.ШаблоныТекстов.ПолучитьТекстШаблона(ШаблонПисьма, Документы.ИсходящееПисьмо.ПустаяСсылка());
...
// иначе помещает их во временное хранилище и заменяет ссылки на временное хранилище.
//
Процедура ВставитьКартинкиВТекстHTMLИзШаблона(ТекстHTML, ШаблонПисьма, УникальныйИдентификаторФормы) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	ВключатьПомеченныеНаУдаление = Ложь;
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ШаблонПисьма, "ПометкаУдаления") Тогда
		ВключатьПомеченныеНаУдаление = Истина;
	КонецЕсли;	
	...
	Запрос = Новый Запрос(
		...
		|ИЗ
		|	Справочник.ШаблоныПисемПрисоединенныеФайлы КАК Файлы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СведенияОФайлах КАК СведенияОФайлах
		|		ПО (СведенияОФайлах.Файл = Файлы.Ссылка)
		|ГДЕ
		|	Файлы.ВладелецФайла = &ШаблонПисьма
		|	И (&ВключатьПомеченныеНаУдаление
		|			ИЛИ НЕ Файлы.ПометкаУдаления)
		|	И Файлы.ИДФайлаЭлектронногоПисьма <> """"");
	Запрос.УстановитьПараметр("ШаблонПисьма", ШаблонПисьма);
	Запрос.УстановитьПараметр("ВключатьПомеченныеНаУдаление", ВключатьПомеченныеНаУдаление);
	Запрос.УстановитьПараметр("ТекущийПользователь", Пользователи.ТекущийПользователь());
	ФайлыПисьма = Запрос.Выполнить().Выгрузить();
	
	ФайлыПисьма.Сортировать("ПорядковыйНомерВложения, Представление");
	...
	Для каждого ФайлПисьма Из ФайлыПисьма Цикл
		...
		Если ПриложениеРаботаетСВебСервером() Тогда
			...
			СсылкаНаКартинку = "data:image/" + Расширение + ";base64," + Base64Строка(ДвоичныеДанныеФайла);
			ТекстHTML = СтрЗаменить(ТекстHTML, "cid:" + ИсточникФайла, СсылкаНаКартинку);
			ТекстHTML = СтрЗаменить(ТекстHTML, "CID:" + ИсточникФайла, СсылкаНаКартинку);
			Если СтрДлина(ИсточникФайла) > 18 Тогда
				ТекстHTML = СтрЗаменить(ТекстHTML, ИсточникФайла, СсылкаНаКартинку);
			КонецЕсли;	
		...
		Иначе // тонкий клиент
			...
			Попытка
				НавигационнаяСсылка = РаботаСФайламиСлужебныйВызовСервера.ПолучитьНавигационнуюСсылкуДляОткрытия(
					ФайлПисьма.Ссылка, // ВерсияСсылка
					УникальныйИдентификаторФормы); // ИдентификаторФормы
			Исключение
				Инфо = ИнформацияОбОшибке();
				ЗаписьЖурналаРегистрации(
			...
			СсылкаНаКартинку = НавигационнаяСсылкаИнформационнойБазы + "/" + НавигационнаяСсылка;
			
			ТекстHTML = СтрЗаменить(ТекстHTML, "http:cid:" + ИсточникФайла, СсылкаНаКартинку); // коррекция ошибочного html
			ТекстHTML = СтрЗаменить(ТекстHTML, "cid:" + ИсточникФайла, СсылкаНаКартинку);
			ТекстHTML = СтрЗаменить(ТекстHTML, "CID:" + ИсточникФайла, СсылкаНаКартинку);
			Если СтрДлина(ИсточникФайла) > 18 Тогда
				ТекстHTML = СтрЗаменить(ТекстHTML, ИсточникФайла, СсылкаНаКартинку);
		...
		ТекстТега = ВыделитьТегImg(ТекстHTML, СсылкаНаКартинку);
		
		Если Не ПустаяСтрока(ТекстТега) Тогда
			
			НавигационнаяСсылкаФайла = "v8doc:" + ПолучитьНавигационнуюСсылку(ФайлПисьма.Ссылка);
...
// Корневыми считаются письма (входящие, исходящие)
...
Функция ПолучитьКорневыеПисьмаПоПредмету(Предмет) Экспорт
	...
	ПравоДоступаЗадачиЧтение = ПравоДоступа("Чтение", Метаданные.Документы.Задачи);
	...
	Если ПравоДоступаЗадачиЧтение Тогда	
		...
		|ИЗ
		|	Документ.Задачи КАК ВходящиеДокументы
...
Функция ТекстЗапросаВычисленияАдресатовДляАвтоподбора()
	...
	ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ОбъектыДляАвтоподбораАдресатов.ОбъектДанных КАК Адресат
		|ИЗ
...
Функция ПолучитьПредставлениеИКонтактАдресата(ПочтовыйАдресат, ПринудительноеДобавлениеАдреса = Ложь) Экспорт
	...
	Запрос.Текст = 
		...
		|					ТОГДА ""0""
		|				КОГДА ТИПЗНАЧЕНИЯ(СведенияОбАдресатах.Контакт) = ТИП(Справочник.Заказчики)
		|					ТОГДА ""2""
		|				КОГДА ТИПЗНАЧЕНИЯ(СведенияОбАдресатах.Контакт) = ТИП(Справочник.КонтактныеЛицаЗаказчиков)
		|					ТОГДА ""3""
...
Функция ПолучитьПредставлениеИКонтактОтправителя(ПочтовыйАдресат, УникальныйИдентификаторФормы, 
	...
	ОтображатьФотографииОбщаяНастройка = Ложь;//ПолучитьФункциональнуюОпцию("ОтображатьФотографииОбщаяНастройка");
	//ПриложениеЯвляетсяВебКлиентом = ОбщегоНазначенияСУЗ.ПриложениеЯвляетсяВебКлиентом();
...
Функция ПолучитьСоответствиеСПредставлениямиИКонтактамиМассиваАдресатов(МассивАдресатов) Экспорт
	...
	Для Каждого СтрокаДанных Из ТаблицаЗначений Цикл
		...
		СтруктураДанных.Вставить("Наименование", СтрокаДанных[4]);
...
Процедура ЗаписатьПротоколДоставкиПочты(Письмо, Текст, ЭтоОшибка, ТипСобытия,
	УчетнаяЗапись, ПорядковыйНомерСобытия, ИдентификаторСеанса) Экспорт
...
Процедура ЗаписатьОшибкуДоставки(СообщениеОбОшибке, Письмо, УчетнаяЗапись, ПараметрыЛогирования,
	ОсталосьПопытокОтправки = Неопределено, ТипСобытияДоставки = Неопределено) Экспорт
	...
	Если ТипСобытияДоставки = Перечисления.ТипыСобытийДоставкиПочты.ОшибкаОтправкиПисьма
		...
		//		Письмо);
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Отправка письма.'", Метаданные.ОсновнойЯзык.КодЯзыка),
		УровеньЖурналаРегистрации.Ошибка,,,
		ТекстУведомления);
...
Процедура ДобавитьПисьмоВПодготовленныеКОтправке(Письмо) Экспорт 
	...
	СтруктураРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Письмо,
		"ПодготовленоКОтправке, УчетнаяЗапись, Автор, ОтправкаОтменена");
...
Функция ПолучитьУчетныеЗаписиПоАдресу(Адрес) Экспорт
...
Функция ПолучитьСписокПодписейПользователя(Знач Пользователь = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстШаблона = Справочники.ШаблоныТекстов.ПолучитьТекстШаблона(Выборка.Ссылка, Документы.ИсходящееПисьмо.ПустаяСсылка());
...
// Обработчик регламентного задания ПриемОтправкаПочты
Процедура ПриемОтправкаПочты() Экспорт
	
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ПриемОтправкаПочты);
	
	ВыполнитьПриемкуОтправкуПочты();
	
КонецПроцедуры

// Принять и отправить почту
Процедура ВыполнитьПриемкуОтправкуПочты() Экспорт
	...
	Запрос.Текст = 
		...
		|	Справочник.УчетныеЗаписиЭлектроннойПочты КАК УчетныеЗаписиЭлектроннойПочты
		|ГДЕ
		|	УчетныеЗаписиЭлектроннойПочты.Актуален
		|	И УчетныеЗаписиЭлектроннойПочты.ВариантИспользования = ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияПочты.Встроенная)
...
Процедура ОтправкаПисем() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОтправкаПисем);
...
Процедура ДобавитьТекстИсходногоПисьма(
	...
	Если ТипТекста = Перечисления.ТипыТекстовПочтовыхСообщений.HTML Тогда
		...
		ИначеЕсли ЭтоШаблон Тогда
			ТекстИсходногоПисьма = HTMLПредставлениеИсходногоПисьма;
		Иначе
...
Процедура ПроверитьНаличиеНовыхПисем(ВозвращаемыеПараметры) Экспорт
	...
	ДатаПоследнейПроверки = НачалоДня(ТекущаяДатаСеанса());
...
Процедура ЗаполнитьДеревоПапок(ДеревоПапок, РежимМоиПапки, ОтображатьУдаленныеПисьмаИПапки) Экспорт
	...
	ТекстЗапроса =
		...
		|	ЕСТЬNULL(ПапкиУчетныхЗаписей.УчетнаяЗапись,Значение(Справочник.УчетныеЗаписиЭлектроннойПочты.ПустаяСсылка)) КАК УчетнаяЗаписьПапки,
		|	ВЫБОР
		...
		|		ПО ПапкиПисем.Ссылка = ПапкиУчетныхЗаписей.Папка
		|ГДЕ
		|	((НЕ ПапкиПисем.ПометкаУдаления)
		|			ИЛИ &ОтображатьУдаленные)
		|	И (ПапкиУчетныхЗаписей.УчетнаяЗапись ЕСТЬ NULL ИЛИ
		|		ПапкиУчетныхЗаписей.УчетнаяЗапись.ОтветственныеЗаОбработкуПисем.Пользователь = &ТекущийПользователь)
		|
		|%ОтборМоиПапки%
		|
		|УПОРЯДОЧИТЬ ПО
...
Процедура ДобавитьПапкиВДерево(СтрокиДерева, СтрокиИсточника, СведенияОПапках)
	...
	Для каждого СтрокаИсточника Из СтрокиИсточника Цикл
		...
		Если Не СтрокаИсточника.УчетнаяЗаписьПапки.Пустая() 
				И Не СтрокаИсточника.УчетнаяЗаписьПапки.Актуален Тогда
			Продолжить;
		КонецЕсли;
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл 
			Если ЗначениеЗаполнено(Выборка.АдресатСообщения) Тогда 
				...
				Если Строка(Выборка.Ссылка) = ФИО Тогда 
					МассивАдресатов = Новый Массив;
...
Функция ПолучитьОсновнойАдрес(Контакт) Экспорт  
	...
	Если ТипЗнч(Контакт) = Тип("СправочникСсылка.Пользователи")
		Или ТипЗнч(Контакт) = Тип("СправочникСсылка.Заказчики") 
		Или ТипЗнч(Контакт) = Тип("СправочникСсылка.КонтактныеЛицаЗаказчиков") 
...
Процедура РазвернутьГруппуВАдресаты(Группа, АдресатыГруппы, ИспользованныеАдреса) Экспорт
	...
	Если ТипЗнч(Группа) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
		...
		Иначе
			...
			Запрос.Текст = 
				...
				|ПОМЕСТИТЬ ГруппыПользователейВременная
				|ИЗ
				...
				|			(ВЫБРАТЬ
				|				ГруппыПользователейВременная.Ссылка
				|			ИЗ
				|				ГруппыПользователейВременная)
				|	И ГруппыПользователейСостав.Пользователь.Недействителен = ЛОЖЬ
				|	И ГруппыПользователейСостав.Пользователь.ПометкаУдаления = ЛОЖЬ
				|
				|УПОРЯДОЧИТЬ ПО
				|	Наименование";
	...
	ИначеЕсли ТипЗнч(Группа) = Тип("СправочникСсылка.Заказчики") Тогда
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	Заказчики.Родитель В ИЕРАРХИИ(&Родитель)
			|	И Заказчики.ЭтоГруппа = ЛОЖЬ
			|	И Заказчики.ПометкаУдаления = ЛОЖЬ
			|
			|УПОРЯДОЧИТЬ ПО
			|	Наименование";
...
// Получает по адресу и представлению структуру с полями Контакт Адресат Адрес 
Функция ПолучитьПараметрыАдресатаПоАдресуИПредставлению(Адрес, Представление) Экспорт
	...
	ОписаниеАдресата = Новый Структура("Контакт, Адресат, Представление, Адрес",
