...
Функция СформироватьСообщение(ПараметрыОтправки) Экспорт
	...
	Если ПараметрыОтправки.Шаблон = Справочники.ШаблоныПисем.ПустаяСсылка() Тогда
	...
	Если ПараметрыШаблона.ТипШаблона = "Письмо" И ПараметрыШаблона.ФорматПисьма = Перечисления.ТипыТекстовПочтовыхСообщений.HTML Тогда
...
Функция СформироватьСообщениеИОтправить(ПараметрыОтправки) Экспорт
	...
	Иначе
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
			...
			Если Сообщение.ДополнительныеПараметры.ФорматПисьма = Перечисления.ТипыТекстовПочтовыхСообщений.HTML Тогда
...
Функция ИспользуютсяШаблоныСообщений() Экспорт
	Возврат ПолучитьФункциональнуюОпцию("ИспользованиеВстроеннойПочты");
КонецФункции
...
Функция СформироватьСообщениеВнешнейОбработкой(ПараметрыШаблона, СведенияОШаблоне, ПараметрыОтправки)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		...
		Если ТипЗнч(Сообщение) = Тип("Структура") И Сообщение.Свойство("СтруктураВложений") Тогда
			...
			СформированноеСообщение.Тема = Сообщение.Тема;
...
Функция ПодготовитьЗапросДляПолученияСпискаШаблонов(ТипШаблона, Предмет = Неопределено, ВладелецШаблона = Неопределено, ВыводитьОбщиеШаблон = Истина) Экспорт
	...
	|	ШаблоныПисем.Ссылка,
	|	ШаблоныПисем.Представление,
	|	ШаблоныПисем.Наименование КАК Имя,
	|	ШаблоныПисем.ВнешняяОбработка КАК ВнешняяОбработка,
	|	ШаблоныПисем.ШаблонПоВнешнейОбработке КАК ШаблонПоВнешнейОбработке,
	|	ВЫБОР
	|		КОГДА ШаблоныПисем.ПредназначенДляЭлектронныхПисем
	|			ТОГДА ВЫБОР
	|					КОГДА ШаблоныПисем.ТипТекста = ЗНАЧЕНИЕ(Перечисление.ТипыТекстовПочтовыхСообщений.HTML)
	|						ТОГДА ШаблоныПисем.ТекстШаблонаПисьмаHTML
	|					ИНАЧЕ ШаблоныПисем.ТекстШаблонаПисьма
	|				КОНЕЦ
	|		ИНАЧЕ ШаблоныПисем.ТекстШаблонаSMS
	|	КОНЕЦ КАК ТекстШаблона,
	|	ШаблоныПисем.ТипТекста,
	|	ШаблоныПисем.Тема,
	|		ВЫБОР
	|			КОГДА КОЛИЧЕСТВО(ШаблоныПисем.Параметры.ИмяПараметра) > 0
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	...
	|ГДЕ
	|	(ШаблоныПисем.ОбщийШаблон ИЛИ ШаблоныПисем.Автор = &Пользователь) 
	|	И ШаблоныПисем.Актуален
	//|	И ШаблоныПисем.ПредназначенДляSMS = &ПредназначенДляSMS
	//|	И ШаблоныПисем.ПредназначенДляЭлектронныхПисем = &ПредназначенДляЭлектронныхПисем
	|	И ШаблоныПисем.Назначение <> ""Служебный""
	|	%ВладелецШаблона%
	|	И ШаблоныПисем.ПометкаУдаления = ЛОЖЬ";
	
	Если ЗначениеЗаполнено(ВладелецШаблона) Тогда
		ОтборПоВладельцу = "И ШаблоныПисем.ВладелецШаблона = &ВладелецШаблона";
	...
	ОтборОбщиеШаблоны = ?(ВыводитьОбщиеШаблон, "ШаблоныПисем.ПредназначенДляВводаНаОсновании = ЛОЖЬ", "");
	
	Если ТипЗнч(Предмет) = Тип("Строка") ИЛИ Предмет = Неопределено Тогда
		Запрос.Текст = Запрос.Текст + ?(ЗначениеЗаполнено(ОтборОбщиеШаблоны), " И " + ОтборОбщиеШаблоны, "");
	Иначе 
		Запрос.Текст = Запрос.Текст + " И (ШаблоныПисем.ПолноеИмяТипаПараметраВводаНаОсновании = &ПолноеИмяТипаПредмета "
...
Функция ЗаписатьВложениеЭлектронногоПисьмаИзВременногоХранилища(Владелец, Сведения, ИмяФайла, Размер, КоличествоПустыхИменВоВложениях = 0) Экспорт
	...
	Возврат РаботаСФайлами.ДобавитьФайл(ПараметрыФайла, АдресВоВременномХранилище, "", Описание);
...
Функция ПоместитьТабличныйДокументВоВременноеХранилище(ПечатнаяФорма, ПараметрыШаблоны, УникальныйИдентификатор)
	...
	ИмяПечатнойформы = ПечатнаяФорма.СинонимМакета;
	Если ТипЗнч(ПечатнаяФорма.ИмяФайлаПечатнойФормы) = Тип("Соответствие") 
			И ПечатнаяФорма.ИмяФайлаПечатнойФормы.Количество() > 0 Тогда
		Для Каждого СтрокаСоответствия Из ПечатнаяФорма.ИмяФайлаПечатнойФормы Цикл
			Если Не ПустаяСтрока(СтрокаСоответствия.Значение) Тогда
				ИмяПечатнойформы = СтрокаСоответствия.Значение;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		Для Каждого ЭлементСписка Из ПараметрыШаблоны.ФорматыВложений Цикл
			...
			ИмяФайла = ПолучитьИмяВременногоФайлаДляПечатнойФормы(ИмяПечатнойформы, НастройкиФормата.Расширение, ИспользованныеИменаФайлов, ПараметрыШаблоны.ТранслитерироватьИменаФайлов);
...
Функция ПараметрыШаблона(Шаблон) Экспорт
	...
	Если ТипЗнч(Шаблон) = Тип("ДанныеФормыСтруктура") Тогда
		...
		КонецЕсли;
		Результат.ФорматПисьма                 = Шаблон.ТипТекста;
		...
		ИначеЕсли Шаблон.ТипТекста = Перечисления.ТипыТекстовПочтовыхСообщений.HTML Тогда
	...
	ИначеЕсли ТипЗнч(Шаблон) = Тип("СправочникСсылка.ШаблоныПисем") Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|	ШаблоныСообщений.ТипТекста,
		|	ВЫБОР
		|		КОГДА ШаблоныСообщений.ПредназначенДляЭлектронныхПисем
		|			ТОГДА ВЫБОР
		|					КОГДА ШаблоныСообщений.ТипТекста = ЗНАЧЕНИЕ(Перечисление.ТипыТекстовПочтовыхСообщений.HTML)
		...
		|ИЗ
		|	Справочник.ШаблоныПисем.Параметры КАК ШаблоныСообщенийПараметры
		|ГДЕ
		...
		Если СведенияОШаблоне.Количество() > 0 Тогда
			...
			КонецЕсли;
			Результат.ФорматПисьма                = СтрокаСведенийОШаблоне.ТипТекста;
			
			Если Результат.ТипШаблона = "SMS" Тогда
				Результат.ПеревестиВТранслит      = СтрокаСведенийОШаблоне.ОтправлятьВТранслите;
			Иначе
				Результат.Тема                    = СтрокаСведенийОШаблоне.Тема;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
		...
		Для каждого Картинка Из ДокументHTML.Картинки Цикл
			...
			Если ИзображениеОтсутствуетВПрисоединенныхФайлах Тогда
				//>> УИТ (добавлена Попытка - Исключение).
				Попытка				
...
Функция СформироватьПолучателейПоУмолчанию(Предмет, ТипШаблона);
	...
	ТипКонтактнойИнформации = Перечисления.ТипыКонтактнойИнформации[?(СтрСравнить(ТипШаблона, "SMS") = 0, "Телефон", "АдресЭлектроннойПочты")];
	...
	Если Получатели.Количество() = 0 И ТипЗнч(Предмет) <> Тип("Строка") Тогда
		...
		ВидыКонтактнойИнформацияОбъекта = УправлениеКонтактнойИнформацией.ВидыКонтактнойИнформацияОбъекта(Предмет);
		Если ВидыКонтактнойИнформацияОбъекта.Количество() > 0 Тогда
			КонтактнаяИнформацияОбъектов = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(ОбъектыСКонтактнойИнформацией, ТипКонтактнойИнформации,, ТекущаяДатаСеанса());
