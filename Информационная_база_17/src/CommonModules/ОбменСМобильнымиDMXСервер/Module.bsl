...
// Проверяет, что задача доступна текущему пользователю в момент обмена.
//
// Параметры:
//  Задача - Документ.Задачи - проверяемая задача.
// 
// Возвращаемое значение:
...
Функция ПолучитьРеквизитыЗадачиСПроверкойДоступности(Задача) Экспорт
	...
	Запрос.Текст = 
		...
		|	Задача.Срок КАК Срок,
		|	Задача.Приоритет КАК Приоритет,
		...
		|	Задача.КонтактноеЛицоЗаказчика КАК КонтактноеЛицоЗаказчика,
		|	Задача.КалендарьСотрудника КАК КалендарьСотрудника
		|ИЗ
		|	Документ.Задачи КАК Задача
		|ГДЕ
		|	Задача.Ссылка = &Ссылка
		|	И Задача.Исполнитель = &Пользователь
		|	И НЕ Задача.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыЗадач.Отменена), ЗНАЧЕНИЕ(Перечисление.СтатусыЗадач.Завершена))";
