...
Процедура ПроверитьВозможностьОтправкиТестовогоСообщения(Знач УчетнаяЗапись, Знач Пароль = Неопределено)
	...
	ПараметрыПисьма.Вставить("Тема", НСтр("ru = 'Тестовое письмо (1С:Система учета задач)'"));
	
	ТекстПисьма = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Это письмо отправлено из конфигурации 1С:Система учета задач для проверки учетной записи %1.
