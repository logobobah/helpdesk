...
Процедура ПриОпределенииПрефиксаИнформационнойБазыПоУмолчанию(Префикс) Экспорт
	...
	// КлиентЭДО начало
	ОбменДаннымиСУЗ.ПриОпределенииПрефиксаИнформационнойБазыПоУмолчанию(Префикс);
	// КлиентЭДО конец
...
Процедура ПолучитьПланыОбмена(ПланыОбменаПодсистемы) Экспорт
	...
	// КлиентЭДО начало
	ОбменДаннымиСУЗ.ПолучитьПланыОбмена(ПланыОбменаПодсистемы);
	// КлиентЭДО конец
...
Процедура РегистрацияИзмененийНачальнойВыгрузкиДанных(Знач Получатель, СтандартнаяОбработка, Отбор) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	Трудозатраты.Дата >= &Дата
	                      |	И Трудозатраты.Проведен
	                      |	И ВЫБОР
	                      |			КОГДА &ЕстьОтборОрганизаций = ИСТИНА
	                      |				ТОГДА Трудозатраты.Организация В (&Организации)
	                      |			ИНАЧЕ ИСТИНА
	                      |		КОНЕЦ");
	Запрос.УстановитьПараметр("Дата", Получатель.ДатаНачалаВыгрузкиДокументов);
	Получатель = ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.НайтиПоКоду("");
	Запрос.УстановитьПараметр("Организации", Получатель.Организации.ВыгрузитьКолонку("Организация"));
	Запрос.УстановитьПараметр("ЕстьОтборОрганизаций", Получатель.ИспользоватьОтборПоОрганизациям);
