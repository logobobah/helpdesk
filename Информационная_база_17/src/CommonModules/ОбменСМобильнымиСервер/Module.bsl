...
Процедура ОбменСМобильнымиПриЗаписиЗадачиПриЗаписи(Источник, Отказ) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.Задачи") Тогда
		//Регистрация задачи для удаления ее в МП при изменении исполнителя
		ЗначенияДоЗаписи = Неопределено;
		Источник.ДополнительныеСвойства.Свойство("ЗначенияДоЗаписи", ЗначенияДоЗаписи);
		Если Источник.Исполнитель <> ЗначенияДоЗаписи.Исполнитель И ЗначениеЗаполнено(ЗначенияДоЗаписи.Исполнитель) Тогда
			МассивПользователей = Новый Массив;
			МассивПользователей.Добавить(ЗначенияДоЗаписи.Исполнитель);
			УзлыОбмена = ОбменСМобильнымиРегистрацияИзмененийСервер.ПолучитьУзлыПоПользователям(МассивПользователей);
			ПланыОбмена.ЗарегистрироватьИзменения(УзлыОбмена,Источник);
		КонецЕсли;
	ИначеЕсли ТипЗнч(Источник) = Тип("СправочникОбъект.ЗаписиКалендаряСотрудника") Тогда
		УзелОбменаДляРегистрацииВсехДанных = ОбменСМобильнымиСерверПовтИсп.ПолучитьУзелДляРегистрацииДанных();
		ПланыОбмена.ЗарегистрироватьИзменения(УзелОбменаДляРегистрацииВсехДанных, Источник);
	КонецЕсли;
...
Функция ПолучитьHTMLПредставлениеФормыЗадачи(СсылкаНаЗадачу) Экспорт
	...
	//Заказчик
	Если ЗначениеЗаполнено(РеквизитыЗадачи.Заказчик) Тогда
		Результат = Результат 
			+ ПолучитьHTMLреквизита(НСтр("ru = 'Заказчик'"), Строка(РеквизитыЗадачи.Заказчик));
	КонецЕсли;
	//Проект
	Если ЗначениеЗаполнено(РеквизитыЗадачи.Проект) Тогда
		Результат = Результат 
			+ ПолучитьHTMLреквизита(НСтр("ru = 'Проект'"), Строка(РеквизитыЗадачи.Проект));
	КонецЕсли;
	//КонтактноеЛицоЗаказчика
	Если ЗначениеЗаполнено(РеквизитыЗадачи.КонтактноеЛицоЗаказчика) Тогда
		Результат = Результат 
			+ ПолучитьHTMLреквизита(НСтр("ru = 'Контактное лицо'"), РеквизитыЗадачи.КонтактноеЛицоЗаказчика);
	КонецЕсли;
	//Статус
	Если ЗначениеЗаполнено(РеквизитыЗадачи.Статус) Тогда
		Результат = Результат 
			+ ПолучитьHTMLреквизита(НСтр("ru = 'Статус'"), РеквизитыЗадачи.Статус);
	КонецЕсли;
	//ОценкаТрудозатрат
	Если ЗначениеЗаполнено(РеквизитыЗадачи.ОценкаТрудозатрат) Тогда
		Результат = Результат 
			+ ПолучитьHTMLреквизита(НСтр("ru = 'Трудоемкость'"), РеквизитыЗадачи.ОценкаТрудозатрат);
	КонецЕсли;
	//Описание
	...
	Если ЗначениеЗаполнено(РеквизитыЗадачи.Срок) Тогда
		Результат = Результат 
			+ ПолучитьHTMLреквизита(
				НСтр("ru = 'Срок'"), Формат(РеквизитыЗадачи.Срок,"ДФ='dd.MM.yyyy ЧЧ:мм'"), Ложь);
	КонецЕсли;

	//Важность
	Если РеквизитыЗадачи.Приоритет <= 3 Тогда
		ПредставлениеВажности = НСтр("ru = 'Высокая'");
	ИначеЕсли РеквизитыЗадачи.Приоритет >= 7 Тогда
	...
	//РезультатВыполнения
	Если ЗначениеЗаполнено(РеквизитыЗадачи.РезультатВыполнения) Тогда
		Результат = Результат 
			+ ПолучитьHTMLреквизита(НСтр("ru = 'Комментарии к выполнению'"), РеквизитыЗадачи.РезультатВыполнения);
	КонецЕсли;

	//Номер
	Результат = Результат 
		+ ПолучитьHTMLреквизита(НСтр("ru = 'Номер'"), РеквизитыЗадачи.Номер, Ложь);
...
Функция ОбъектДоступенТекущемуПользователю(МобильныйКлиент, Объект, ОбщиеПраваНаОбъекты = Неопределено) Экспорт
	...
	Попытка
		...
		//ПраваПоОбъекту = СУЗПраваДоступа.ПолучитьПраваПоОбъекту(
		//	Объект, Пользователь);
...
Функция ПолучитьПараметрыСинхронизации(Пользователь) Экспорт
	...
	Если ПараметрыСинхронизации.ПодробныйПротоколОбмена Тогда
		...
		ПараметрыСинхронизации.ПротоколируемыеТипы.Вставить(
			Тип("ДокументСсылка.Задачи"), Тип("ДокументСсылка.Задачи"));

		ПараметрыСинхронизации.ПротоколируемыеТипы.Вставить(
			Тип("СправочникСсылка.ЗаписиКалендаряСотрудника"), Тип("СправочникСсылка.ЗаписиКалендаряСотрудника"));
