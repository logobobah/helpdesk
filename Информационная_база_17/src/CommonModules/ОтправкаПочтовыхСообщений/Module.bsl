...
//
// Параметры:
//  Заказчик - СправочникСсылка.Заказчики, Массив - ссылка на контрагента или массив ссылок.
//
...
Функция АдресаЭлектроннойПочты(Заказчик) Экспорт
	...
	Получатели = ОтправкаПочтовыхСообщенийПереопределяемый.АдресаЭлектроннойПочты(Заказчик);
...
Функция ПараметрыЭлектронногоПисьмаДляОтчетов(ПараметрыОтчета, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		...
		Если ДополнительныеПараметры.Свойство("Заказчик") Тогда
			Получатель = АдресаЭлектроннойПочты(ДополнительныеПараметры.Заказчик);
		КонецЕсли;
...
//
// Параметры:
//  Ссылка - СправочникСсылка.Заказчики, СправочникСсылка.Организации, СправочникСсылка.ФизическиеЛица.
//
// Возвращаемое значение:
//  Строка - Наименование.
//
Функция НаименованиеЗаказчика(Ссылка) Экспорт
	...
	ИменаРеквизитовОбъекта.Вставить(Тип("СправочникСсылка.Заказчики")   , "Наименование");
...
Процедура ПослеОтправкиПисьма(ПараметрыПисьма) Экспорт
	...
	Заказчик = ПолучателиСообщения[0].ИсточникКонтактнойИнформации;
	Адреса = Новый Массив;
	...
	Если Адреса.Количество() > 0 Тогда
		СохранитьАдресаЭлектроннойПочты(Заказчик, Адреса);
	КонецЕсли;
...
Функция НовыеПараметрыПолучателя() Экспорт
	...
	Результат.Вставить("ИсточникКонтактнойИнформации", Справочники.Заказчики.ПустаяСсылка());
...
Процедура УстановитьПолучателяПоУмолчанию(Контакты) Экспорт
	...
	Заказчики = ОбщегоНазначения.ВыгрузитьКолонку(Контакты, "ИсточникКонтактнойИнформации", Истина);
	...
	Для каждого Заказчик Из Заказчики Цикл
		...
		//ОсновноеКонтактноеЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Заказчик, "ОсновноеКонтактноеЛицо");
		//
		...
		Контакт = Контакты.Найти(Заказчик, "КонтактноеЛицо");
		Если Контакт <> Неопределено Тогда
			Контакт.Выбран = Истина;
			Продолжить;
		КонецЕсли;
		...
		Иначе
			КонтактыКонтрагента = Контакты.НайтиСтроки(Новый Структура("ИсточникКонтактнойИнформации", Заказчик));
...
Функция ИмяФайлаПечатнойФормы(ОбъектПечати, НазваниеПечатнойФормы, Расширение)
	...
	Если ОбщегоНазначения.ЭтоДокумент(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектПечати))) Тогда
		...
		ПараметрыДляВставки.Номер = ПараметрыДляВставки.Номер;
...
Процедура СохранитьАдресаЭлектроннойПочты(Заказчик, Адреса)
	
	Если Не ЗначениеЗаполнено(Заказчик) Тогда
		Возврат;
	КонецЕсли;
	
	ВидКИ = Справочники.ВидыКонтактнойИнформации.EmailЗаказчика;
	
	ДоступенВвводНесколькихЗначений = ВидКИ.РазрешитьВводНесколькихЗначений;
	
	ПараметрыОтбора = Новый Структура("Вид", ВидКИ);
	НайденныеСтрокиКИ = Заказчик.КонтактнаяИнформация.НайтиСтроки(ПараметрыОтбора);
	...
	ИначеЕсли НЕ ДоступенВвводНесколькихЗначений Тогда
		УправлениеКонтактнойИнформацией.УстановитьКонтактнуюИнформациюОбъекта(Заказчик, ВидКИ, Адреса[0]);
		Возврат;
	КонецЕсли;
	
	Приемник = Заказчик.ПолучитьОбъект();
...
Процедура ЗаполнитьТемуТелоПисьмаСДокументом(МассивОбъектов, ПараметрыПисьма, КоллекцияПечатныхФорм, КоличествоЭД)
	...
	СтруктураТиповОбъектыПечати = ОбщегоНазначенияСУЗ.РазложитьСписокПоТипамОбъектов(МассивОбъектов);
	
	Отправитель = ПолучитьЗначениеОбщегоРеквизита(СтруктураТиповОбъектыПечати, "Организация", ОписаниеТиповИзСтроки("Организации"));
	
	ОписаниеДопустимыхТиповКонтрагента = ОписаниеТиповИзСтроки("Заказчики");
	Получатель  = ПолучитьЗначениеОбщегоРеквизита(СтруктураТиповОбъектыПечати, "Заказчик", ОписаниеДопустимыхТиповКонтрагента);
	...
	Если Получатель <> Неопределено Тогда
		ПредставлениеПолучателя = ПолучитьПредставлениеСПрефиксом(НСтр("ru='для'"),
			НаименованиеЗаказчика(Получатель));
	КонецЕсли;
	...
	Если Отправитель <> Неопределено Тогда
		ПредставлениеОтправителя = ПолучитьПредставлениеСПрефиксом(НСтр("ru='от'"),
			НаименованиеЗаказчика(Отправитель));
	КонецЕсли;
	...
	Для Каждого Объект Из МассивОбъектов Цикл
		...
		Иначе
			...
			Для Каждого ПечатнаяФорма Из СведенияОбъектаПечати.ПечатныеФормы Цикл
				...
				ОбщегоНазначенияСУЗВызовСервера.СформироватьЗаголовокДокумента(Объект, ПечатнаяФорма.Ключ.Представление),
					ТекстСуммаДокумента);
			КонецЦикла;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ПутьКДанным) Тогда
			Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
				ТекстПисьма = ТекстПисьма + Символы.ПС + СтрШаблон(НСтр("ru = '- %1 %2'"),
					ОбщегоНазначенияСУЗВызовСервера.СформироватьЗаголовокДокумента(Объект, ПечатнаяФорма[ПутьКДанным]),
					ТекстСуммаДокумента);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
