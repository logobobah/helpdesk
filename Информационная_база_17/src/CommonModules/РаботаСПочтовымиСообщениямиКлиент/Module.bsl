...
Процедура ОткрытьФормуОтправкиПочтовогоСообщения(знач Отправитель = Неопределено,
	...
	ИспользованиеЛегкойПочты = Ложь;//ПолучитьФункциональнуюОпциюИнтерфейса("ИспользованиеЛегкойПочты");
	...
	Если ИспользованиеВстроеннойПочты Тогда		
		...
		ПараметрыОткрытия.Вставить("Отправитель",Отправитель);
		ПараметрыОткрытия.Вставить("Тема",Тема);
		ПараметрыОткрытия.Вставить("Текст",Текст);
		ПараметрыОткрытия.Вставить("УдалятьФайлыПослеОтправки",УдалятьФайлыПослеОтправки);
		ПараметрыОткрытия.Вставить("ПисьмоДолжноСохраняться",ПисьмоДолжноСохраняться);
			...
			Если ТипЗнч(Получатель) = Тип("Массив") Тогда
				Для Каждого ЭлементМассива Из Получатель Цикл
					АдресИнфо = Новый Структура("Контакт, Адрес, ОтображаемоеИмя",
						"", ЭлементМассива.Адрес, ЭлементМассива.Представление);
...
Процедура СоздатьНовоеПисьмо(ПараметрыОтправки = Неопределено, ОповещениеОЗакрытииФормы = Неопределено) Экспорт
	...
	Тема = "";
	Если ПараметрыОтправки.Свойство("Тема") Тогда
		Тема = ПараметрыОтправки.Тема;
	КонецЕсли;	
	...
	УдалятьФайлыПослеОтправки = Ложь;
	Если ПараметрыОтправки.Свойство("УдалятьФайлыПослеОтправки") Тогда
		УдалятьФайлыПослеОтправки = ПараметрыОтправки.УдалятьФайлыПослеОтправки;
	КонецЕсли;	
	
	//Письмо привязывается к первому документу из которого формируется письмо
	Основание = Неопределено;
	Если ПараметрыОтправки.Свойство("АдресИсходногоВложения") Тогда
		ДопПараметры = ПолучитьИзВременногоХранилища(ПараметрыОтправки.АдресИсходногоВложения);
		Если ДопПараметры.НаборПечатныхФорм.Количество()>0 Тогда
			Если ДопПараметры.НаборПечатныхФорм[0].ОбъектыПечати.Количество()>0 Тогда
				Основание = ДопПараметры.НаборПечатныхФорм[0].ОбъектыПечати[0].Значение;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;	
	
	ОткрытьФормуОтправкиПочтовогоСообщения(Отправитель, Получатель, Тема, Текст, Вложения, УдалятьФайлыПослеОтправки, , Основание);
