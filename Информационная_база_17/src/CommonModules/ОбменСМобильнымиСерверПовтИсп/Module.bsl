...
Функция ПолучитьПредставлениеТипаПоСсылке(ЭлементДанных) Экспорт
	...
	ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("СправочникСсылка.ЗаписиКалендаряСотрудника") Тогда
		Тип = "CalendarEvent";

	ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("ДокументСсылка.Задачи") Тогда
		Тип = "Task";
...
Функция ПолучитьСсылкуПоТипу(ТипЗначения, СтрокаИдентификатора) Экспорт
	...
	ИначеЕсли СтрСравнить(ТипЗначения,"Task") = 0 Тогда
		Возврат Документы.Задачи.ПолучитьСсылку(Идентификатор);

	ИначеЕсли СтрСравнить(ТипЗначения,"CalendarEvent") = 0 Тогда
		Возврат Справочники.ЗаписиКалендаряСотрудника.ПолучитьСсылку(Идентификатор);
...
//
Функция ПолучитьИдентификаторВариантаВыполненияЗадачи(Задача) Экспорт

	РезультатВыполнения = "NotExecuted";

	Если Задача.Статус = Перечисления.СтатусыЗадач.Завершена Тогда
		РезультатВыполнения = "ExecutedPositive";

	ИначеЕсли Задача.Статус = Перечисления.СтатусыЗадач.Отменена Тогда
...
//  Строка - Строковое представление значения перечисления для обмена.
//
Функция ПолучитьПредставлениеСостоянияЗаписиКалендаряСотрудника(Состояние) Экспорт
