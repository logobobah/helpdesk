	...
	Если Контекст.ТребуетсяОпределитьСсылки Тогда
		...
		Иначе
			Для Каждого Идентификатор Из Источник.ВыделенныеСтроки Цикл
				//Изменен стандартный алгоритм БСП, что бы формрвоать печатные формы можно было не только из форм списка с видимой колонкой "Ссылка"
				//ДанныеСтроки = Источник.ДанныеСтроки(Идентификатор);
				ДанныеСтроки = Новый Структура;
				ДанныеСтроки.Вставить("Ссылка",Идентификатор);
				ДобавитьСсылкуВСписок(ДанныеСтроки, МассивСсылок, ОписаниеКоманды.ТипПараметра);
			КонецЦикла;
		КонецЕсли;
