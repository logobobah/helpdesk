...
Функция ПолучитьСписокПочтовыхАдресовЗаказчика(Заказчик)
	...
	Запрос = Новый Запрос(
		...
		|ИЗ
		|	Справочник.Заказчики.КонтактнаяИнформация КАК КонтрагентыКонтактнаяИнформация
		|ГДЕ
	...
	Запрос.УстановитьПараметр("Ссылка", Заказчик);
	...
	Пока Выборка.Следующий() Цикл
		Если ЗначениеЗаполнено(Выборка.Представление) Тогда
			...
			СтруктураАдреса.Контакт = Заказчик;
			СтруктураАдреса.ОтображаемоеИмя = Строка(Заказчик);
...
// Параметры:
//  Данные    - ссылка письма, выводимого в списке связанных документов.
...
Процедура ПредставлениеВСтруктуреПодчиненности(Данные, Представление, СтандартнаяОбработка) Экспорт
	СтандартнаяОбработка = Ложь;
	Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Исходящее письмо: %1 от %2'"), Данные.Тема, Строка(Данные.Дата));
КонецПроцедуры
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Основание")
		И ТипЗнч(Параметры.Основание) = Тип("СправочникСсылка.Заказчики") Тогда
		СписокПочтовыхАдресов = ПолучитьСписокПочтовыхАдресовЗаказчика(Параметры.Основание);
