...
&НаКлиенте
Процедура ЗавершениеСоздатьКонтактноеЛицо(Заказчик, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(Заказчик) Тогда
		...
		Адрес = ОбщегоНазначенияСУЗВызовСервера.ЗначениеРеквизитаОбъекта(
		...
		ОткрытьФорму("Справочник.КонтактныеЛицаЗаказчиков.ФормаОбъекта", ПараметрыФормы);
...
Процедура УстановитьДоступностьКомандСохраненияКонтакта(Доступность)
	 ...
	 //Элементы.ФормаСоздатьЗаказчика.Доступность = Доступность;
...
Функция ПолучитьПодписьПриОтветеИПересылке()
	...
	Если ТипЗнч(ПодписьПриОтветеИПересылке) = Тип("СправочникСсылка.ШаблоныТекстов")
		...
		Подпись = Справочники.ШаблоныТекстов.ПолучитьТекстШаблона(ПодписьПриОтветеИПересылке, Документы.ИсходящееПисьмо.ПустаяСсылка());
