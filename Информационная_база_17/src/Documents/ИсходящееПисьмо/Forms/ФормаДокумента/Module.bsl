...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.ПисьмоОснование) И 
		...
		(Объект.ТипОтвета = Перечисления.ТипыОтвета.ОтветНаПисьмо Или 
		 Объект.ТипОтвета = Перечисления.ТипыОтвета.ПересылкаПисьма) Тогда
	...
	Если ОбщегоНазначенияСУЗ.ПриложениеЯвляетсяВебКлиентом() Тогда
	...
	Если ПисьмоОтправлено Или ОбщегоНазначенияСУЗ.ПриложениеЯвляетсяВебКлиентом() Тогда
	...
	СписокВыбора.Добавить(ПредопределенноеЗначение("Документ.Трудозатраты.ПустаяСсылка"), НСтр("ru = 'Трудозатраты'"));
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	ОбщегоНазначенияСУЗКлиент.ВставитьВОписаниеОповещенияОЗакрытииСсылкуНаОбъект(ЭтаФорма);
...
Процедура ЗаполнитьНовоеПисьмо(Параметры)
	...
	ИначеЕсли Параметры.Свойство("Основание")
		...
		Если ТипЗнч(Параметры.Основание) = Тип("СправочникСсылка.ШаблоныПисем") Тогда 
			...
			ПараметрыШаблона = ШаблоныСообщений.СформироватьСообщение(Параметры.Основание, Параметры.Основание.Предмет, УникальныйИдентификатор);
			ПараметрыШаблона.Вставить("УдалятьФайлыПослеОтправки", Ложь);
			ПараметрыШаблона.Вставить("Шаблон", Параметры.Основание);
			
			МассивВложений = Новый Массив;
			Для Каждого ОписаниеВложения Из ПараметрыШаблона.Вложения Цикл
			...
			КонецЦикла;
			ПараметрыШаблона.Вставить("Вложения", МассивВложений);
			
			ЗаполнитьПоШаблонуНаСервере(ПараметрыШаблона, Ложь);
			
		Иначе 
			
			ТекстПисьма = ?(Параметры.Свойство("Текст") и Не ПустаяСтрока(Параметры.Текст), Параметры.Текст, 
				ВстроеннаяПочтаСервер.СформироватьТекстИсходящегоПисьма(
					Параметры.Основание,
					Объект.ТипТекста,
					Объект.Кодировка,
					Объект.ТипОтвета)
...
Процедура ВыбратьИзАдреснойКниги(ПараметрМножественныйВыбор = Неопределено)
	...
	ПараметрыОткрытия.Вставить("ВыбиратьЗаказчиков", Истина);
	ПараметрыОткрытия.Вставить("ВыбиратьГруппыПользователей", Истина);
...
Функция ЗанестиВПолучателиВыбранныеСтроки(ВыбранныеАдресаты, ИдТекущаяСтрока, 
	...
	Для Индекс = 0 По ВыбранныеАдресаты.Количество() - 1 Цикл
		...
		Если Строка.Свойство("Адресат") И ЗначениеЗаполнено(Строка.Адресат) Тогда 
...
Процедура СоздатьЛичногоАдресата(Команда)
	...
	Если Элементы.Получатели.ТекущиеДанные <> Неопределено
		...
		Адрес = ОбщегоНазначенияСУЗВызовСервера.ЗначениеРеквизитаОбъекта(Элементы.Получатели.ТекущиеДанные.Адресат, "Адрес");
...
&НаКлиенте
Процедура СоздатьЗаказчика(Команда)
	...
	Если Элементы.Получатели.ТекущиеДанные <> Неопределено
		...
		ОткрытьФорму(
			"Справочник.Заказчики.ФормаОбъекта", 
			ПараметрыФормы,
			ЭтаФорма,,,,,
			РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура СоздатьКонтактноеЛицо(Команда)
	...
	Если Элементы.Получатели.ТекущиеДанные <> Неопределено
		...
		ОткрытьФорму(
			"Справочник.Заказчики.ФормаВыбора",
			,
			ЭтаФорма,,,,
			ОписаниеОповещения,
			РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
&НаКлиенте
Процедура ЗавершениеСозданияКонтактногоЛица(Заказчик, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(Заказчик) Тогда
		...
		ПараметрыФормы = Новый Структура("Адрес, Представление, Заказчик", 
			Адрес,
			Представление,
			Заказчик);
		...
		ОткрытьФорму(
			"Справочник.КонтактныеЛицаЗаказчиков.ФормаОбъекта", 
			ПараметрыФормы,
			ЭтаФорма,,,,,
			РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ВставитьТекущуюДатуИВремя(Команда)
	...
	Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML") Тогда
		...
		Узел = Выделение.createContextualFragment(СтрокаВставки);
...
Процедура ЗаполнитьПоШаблону(Команда)
	...
	ШаблоныСообщенийКлиент.ПодготовитьСообщениеПоШаблону(Объект.Предмет, "Письмо", ОписаниеОповещения);
...
//Результат - структура с полями Вложения, Предмет, СообщенияПользователю, Текст, Тема, УдалятьФайлыПослеОтправки, Шаблон
&НаСервере
Процедура ЗаполнитьПоШаблонуНаСервере(Результат, ДобавитьПолучателей = Истина)
	
	//Получатели.Очистить();
	ТекстПисьма = "";
	
	РеквизитыОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Результат.Шаблон,
			"ТипТекста, Кодировка, Проект, Важность, УчетнаяЗапись");
	...
	Если ЗначениеЗаполнено(Результат.Тема) Тогда 
	...
	Объект.УчетнаяЗапись = РеквизитыОснования.УчетнаяЗапись;
	
	//Добавление получателей
	Если ДобавитьПолучателей Тогда
		ТаблицаПолучателиПисьма = Результат.Шаблон.ПолучателиПисьма.Выгрузить();
		ТаблицаПолучателиКопий = Результат.Шаблон.ПолучателиКопий.Выгрузить();
		ТаблицаПолучателиОтвета = Результат.Шаблон.ПолучателиОтвета.Выгрузить();
		ТаблицаПолучателиСкрытыхКопий = Результат.Шаблон.ПолучателиСкрытыхКопий.Выгрузить();
		
		Если Результат.Шаблон.ГруппыПолучателей.Количество() > 0 Тогда 
			Счетчик = 0;
			Для Каждого Шаблон Из Результат.Шаблон.ГруппыПолучателей Цикл 
	...
	ТипТекста = Перечисления.ТипыТекстовПочтовыхСообщений.HTML;
	ТекстПисьма = ?(ТипЗнч(Результат.Текст) = Тип("Структура"),Результат.Текст.ТекстHTML, Результат.Текст);
	Если СтрНачинаетсяС(ТекстПисьма, "<!DOCTYPE html") Или СтрНачинаетсяС(ТекстПисьма, "<html") Тогда
		//ТекстИсходногоПисьма = РаботаС_HTML.РазложитьТекстHTML(ТекстПисьма).Тело;
		...
		Если Объект.ТипТекста = ПростойТекст Тогда
			ТекстПисьма = РаботаС_HTML.ПолучитьПростойТекстИзHTML(ТекстПисьма);
		КонецЕсли;
	Иначе
		Если Объект.ТипТекста = HTML Тогда
	...
	//Добавление вложений
	Для Каждого СтрокаМассива Из Результат.Вложения Цикл
			
		Размер = ПолучитьРазмерФайлаВоВременномХранилище(СтрокаМассива.АдресВоВременномХранилище);
		
		ИмяФайлаИнфо = РаботаСоСтроками.РазложитьПолноеИмяФайла(СтрокаМассива.Представление);
		
		//Определение картинки из текста
		Если ПустаяСтрока(СтрокаМассива.Идентификатор) Тогда
			ВложенияСтрока = Вложения.Добавить();
		ИначеЕсли СтрНайти(ТекстПисьма, СтрокаМассива.Представление) > 0 Тогда
			ТекстПисьма = СтрЗаменить(ТекстПисьма, СтрокаМассива.Представление, СтрокаМассива.Идентификатор);
			ВложенияСтрока = ВложенияКартинки.Добавить();
		ИначеЕсли СтрНайти(ТекстПисьма, СтрокаМассива.Идентификатор) > 0 Тогда
			ВложенияСтрока = ВложенияКартинки.Добавить();
		Иначе
			ВложенияСтрока = Вложения.Добавить();
		КонецЕсли;
		...
		ВложенияСтрока.ИмяФайлаНаДиске = СтрокаМассива.Представление;
		...
		ВложенияСтрока.РедактируетТекущийПользователь = Ложь;
		ВложенияСтрока.Расположение = "ВременноеХранилище";
		ВложенияСтрока.Адрес = ПоместитьВоВременноеХранилище(
				ПолучитьИзВременногоХранилища(СтрокаМассива.АдресВоВременномХранилище), УникальныйИдентификатор);
	КонецЦикла;
	ОтобразитьКоличествоФайловСервер();

	//Вставка картинок в HTML
	ВстроеннаяПочтаСервер.ВставитьКартинкиВТекстHTMLИзШаблона(ТекстПисьма, Результат.Шаблон, УникальныйИдентификатор);
...
Процедура ВложениеПометитьНаУдалениеСервер(ФайлСсылка, РедактируетТекущийПользователь, ПометкаУдаления)
	...
	Если Не ФайлСсылка.Пустая() Тогда
...
Процедура ПолучателиПредставлениеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		Иначе
			СтрокаДанных.Адрес = 
				ОбщегоНазначенияСУЗВызовСервера.ЗначениеРеквизитаОбъекта(СтрокаДанных.Адресат, "Адрес");
		КонецЕсли;
		СтрокаДанных.РедактированиеЗавершено = Истина;
		Модифицированность = Истина;
...
Процедура УстановитьДоступностьКомандСохраненияКонтакта(Доступность)
	...
	Элементы.ПолучателиПредставлениеКонтекстноеМенюСоздатьЗаказчика.Доступность = Доступность;
...
Функция ПолучитьАктивныеЗадачиПоПисьму(Письмо)
	...
	Запрос = Новый Запрос(
		...
		|	И НЕ (ЗадачаИсполнителя.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыЗадач.Завершена),ЗНАЧЕНИЕ(Перечисление.СтатусыЗадач.Отменена)))");
...
Функция ПолучитьТекстШаблона_ТипТекста(Шаблон, ТипТекста)
	...
	ТипТекста = ОбщегоНазначенияСУЗВызовСервера.ЗначениеРеквизитаОбъекта(
		Шаблон, "ТипТекста");
...
Процедура ЗавершениеВставкиШаблонаТекста(ШаблонСсылка, Параметры) Экспорт
	...
	Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML") Тогда
		...
		Выделение = HTMLДокумент.getSelection().getRangeAt(0);
		Узел = Выделение.createContextualFragment(ШаблонТекста);
		Выделение.insertNode(Узел);
...
Процедура ВключитьРежимРедактированияHTML()
	...
	Если Объект.ТипТекста = Перечисления.ТипыТекстовПочтовыхСообщений.HTML И Не ПисьмоОтправлено Тогда
...
Процедура ВставитьБезФорматирования(Команда)
	...
	Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML") Тогда
		...
		Если ЗначениеЗаполнено(ТекстИзБуфера) Тогда
			...
			Выделение = HTMLДокумент.getSelection().getRangeAt(0);
			Узел = Выделение.createContextualFragment(ТекстИзБуфера);
			Выделение.insertNode(Узел);
		КонецЕсли;	
...
Процедура ПредметСтрокойНажатиеЗавершениеВыбораТипа(Результат, ДополнительныеПараметры) Экспорт
	...
	ПараметрыФормы = Новый Структура("РежимВыбора, ЗакрыватьПриВыборе, ЗакрыватьПриЗакрытииВладельца", Истина, Истина, Истина);
	ОткрытьФорму(ПолноеИмя + ".ФормаВыбора", ПараметрыФормы, , , , , ОписаниеОповещения, РежимОткрытияОкна);
